...
for (i, (train_res, truth_train)) in enumerate(zip(training_results, training_data_plot.data))
    ...
    with_theme(theme_latexfonts()) do
        ...
        n = Observable(1)
        time_str = @lift "NN Only Training: Qᵁ = $(Qᵁ) m² s⁻², Qᵀ = $(Qᵀ) m s⁻¹ °C, Qˢ = $(Qˢ) m s⁻¹ g kg⁻¹, f = $(f) s⁻¹, Time = $(round(times[$n], digits=1)) days"
        ...
        linkyaxes!(axT, axS, axσ, axwT, axwS, axRi)
        ...
        # display(fig)
        CairoMakie.record(fig, "./slides_figures/slightlylocalNN/training_$(i).mp4", 1:Nt, framerate=20) do nn
            n[] = nn
        end
    end
end
...
for (i, (train_res, truth_train)) in enumerate(zip(interpolating_results, interpolating_data_plot.data))
    ...
    with_theme(theme_latexfonts()) do
        ...
        n = Observable(1)
        time_str = @lift "NN Only Validation: Qᵁ = $(Qᵁ) m² s⁻², Qᵀ = $(Qᵀ) m s⁻¹ °C, Qˢ = $(Qˢ) m s⁻¹ g kg⁻¹, f = $(f) s⁻¹, Time = $(round(times[$n], digits=1)) days"
        ...
        # display(fig)
        CairoMakie.record(fig, "./slides_figures/slightlylocalNN/validation_$(i).mp4", 1:Nt, framerate=20) do nn
            n[] = nn
        end
    end
end
#%%
for (i, (train_res, truth_train, loss)) in enumerate(zip(training_results, training_data_plot.data, weighted_training_loss))
    @info "Plotting $i"
    ...
    with_theme(theme_latexfonts()) do
        ...
        lines!(axT, T_predₙ, zC, color=:black, linewidth=3, label="Neural Networks Only")
        lines!(axS, S_predₙ, zC, color=:black, linewidth=3, label="Neural Networks Only")
        lines!(axσ, ρ_predₙ, zC, color=:black, linewidth=3, label="Neural Networks Only")

        lines!(axwT, wT_truthₙ, zF[1:end-1], color=(colors[1], 0.5), linewidth=5, label="LES Solution")
        lines!(axwS, wS_truthₙ, zF[1:end-1], color=(colors[1], 0.5), linewidth=5, label="LES Solution")

        lines!(axwT, wT_predₙ, zF[1:end-1], color=:black, linewidth=3, label="Neural Networks Only")
        lines!(axwS, wS_predₙ, zF[1:end-1], color=:black, linewidth=3, label="Neural Networks Only")

        lines!(axloss, times, loss, linewidth=3, color=:black, label="Neural Networks Only")
        ...
        # display(fig)
        CairoMakie.record(fig, "./slides_figures/slightlylocalNN_simple/training_$(i).mp4", 1:Nt, framerate=20) do nn
                n[] = nn
        end
    end
end
#%%
for (i, (train_res, truth_train, loss)) in enumerate(zip(interpolating_results, interpolating_data_plot.data, weighted_interpolating_loss))
    @info "Plotting $i"
    ...
    with_theme(theme_latexfonts()) do
        ...
        # display(fig)
        CairoMakie.record(fig, "./slides_figures/slightlylocalNN_simple/validation_$(i).mp4", 1:Nt, framerate=20) do nn
                n[] = nn
        end
    end
end
