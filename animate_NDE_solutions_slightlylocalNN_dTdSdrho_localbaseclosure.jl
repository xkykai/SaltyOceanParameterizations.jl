...
FILE_DIR = "./final_training_output/finallocalfullrun_slightlylocalNN_dTSrho/NDE_enzyme_2layer_64_relu_1.0Sscaling/training_results_epoch20000_end285.jld2"
...
jldsave("./plots_data/slightlylocalNN_localbaseclosure_dTdSdrho_training_results.jld2", u=training_results)
jldsave("./plots_data/slightlylocalNN_localbaseclosure_dTdSdrho_interpolating_results.jld2", u=interpolating_results)
...
individual_training_losses_noNN = [individual_loss(res.sols_dimensional_noNN, truth, params.Dᶠ, scaling) for (res, truth, params) in zip(training_results, training_data_plot.data, training_params)]
individual_interpolating_losses_noNN = [individual_loss(res.sols_dimensional_noNN, truth, params.Dᶠ, scaling) for (res, truth, params) in zip(interpolating_results, interpolating_data_plot.data, interpolating_params)]

weighted_training_loss_prefactors_noNN = compute_loss_prefactor_density_contribution.(individual_training_losses_noNN, density_contribution_training)
weighted_training_loss_prefactors_noNN = compute_loss_prefactor_density_contribution.(individual_interpolating_losses_noNN, density_contribution_interpolating)

weighted_training_losses_noNN = [(; T=weighted_training_loss_prefactors_noNN[i].T * individual_training_losses_noNN[i].T,
                            S=weighted_training_loss_prefactors_noNN[i].S * individual_training_losses_noNN[i].S,
                            ρ=weighted_training_loss_prefactors_noNN[i].ρ * individual_training_losses_noNN[i].ρ,
                            ∂T∂z=weighted_training_loss_prefactors_noNN[i].∂T∂z * individual_training_losses_noNN[i].∂T∂z,
                            ∂S∂z=weighted_training_loss_prefactors_noNN[i].∂S∂z * individual_training_losses_noNN[i].∂S∂z,
                            ∂ρ∂z=weighted_training_loss_prefactors_noNN[i].∂ρ∂z * individual_training_losses_noNN[i].∂ρ∂z) for i in 1:length(training_results)]

weighted_interpolating_losses_noNN = [(; T=weighted_training_loss_prefactors_noNN[i].T * individual_interpolating_losses_noNN[i].T,
                            S=weighted_training_loss_prefactors_noNN[i].S * individual_interpolating_losses_noNN[i].S,
                            ρ=weighted_training_loss_prefactors_noNN[i].ρ * individual_interpolating_losses_noNN[i].ρ,
                            ∂T∂z=weighted_training_loss_prefactors_noNN[i].∂T∂z * individual_interpolating_losses_noNN[i].∂T∂z,
                            ∂S∂z=weighted_training_loss_prefactors_noNN[i].∂S∂z * individual_interpolating_losses_noNN[i].∂S∂z,
                            ∂ρ∂z=weighted_training_loss_prefactors_noNN[i].∂ρ∂z * individual_interpolating_losses_noNN[i].∂ρ∂z) for i in 1:length(interpolating_results)]
...
weighted_training_loss_noNN = sum.(values(weighted_training_losses_noNN))
weighted_interpolating_loss_noNN = sum.(values(weighted_interpolating_losses_noNN))
...
for (i, (train_res, truth_train)) in enumerate(zip(training_results, training_data_plot.data))
    ...
    with_theme(theme_latexfonts()) do
        fig = Figure(size=(1400, 980), fontsize=20)
        ...
        axRi = CairoMakie.Axis(l1[1,4], ylabel="z (m)", xlabel=L"Ri")
        ...
        wTlim = (find_min(truth_train.flux.wT.column.unscaled[:, 50:end], train_res.fluxes.wT.residual[:, 50:end]),
                 find_max(truth_train.flux.wT.column.unscaled[:, 50:end], train_res.fluxes.wT.residual[:, 50:end]))
        wSlim = (find_min(truth_train.flux.wS.column.unscaled[:, 50:end], train_res.fluxes.wS.residual[:, 50:end]),
                 find_max(truth_train.flux.wS.column.unscaled[:, 50:end], train_res.fluxes.wS.residual[:, 50:end]))
        ...
        n = Observable(1)
        time_str = @lift "NN + Local Base Closure Training: Qᵁ = $(Qᵁ) m² s⁻², Qᵀ = $(Qᵀ) m s⁻¹ °C, Qˢ = $(Qˢ) m s⁻¹ g kg⁻¹, f = $(f) s⁻¹, Time = $(round(times[$n], digits=1)) days"
        ...
        T_noNNₙ = @lift train_res.sols_dimensional_noNN.T[:, $n]
        S_noNNₙ = @lift train_res.sols_dimensional_noNN.S[:, $n]
        ρ_noNNₙ = @lift train_res.sols_dimensional_noNN.ρ[:, $n]

        κ_predₙ = @lift train_res.diffusivities.κ[2:end-1, $n]
        κ_noNNₙ = @lift train_res.diffusivities_noNN.κ[2:end-1, $n]
        ...
        wT_residualₙ = @lift train_res.fluxes.wT.residual[1:end-1, $n]
        wS_residualₙ = @lift train_res.fluxes.wS.residual[1:end-1, $n]

        wT_diffusive_boundaryₙ = @lift train_res.fluxes.wT.diffusive_boundary[1:end-1, $n]
        wS_diffusive_boundaryₙ = @lift train_res.fluxes.wS.diffusive_boundary[1:end-1, $n]

        wT_noNNₙ = @lift train_res.fluxes_noNN.wT.total[1:end-1, $n]
        wS_noNNₙ = @lift train_res.fluxes_noNN.wS.total[1:end-1, $n]
        ...
        Ri_noNNₙ = @lift clamp.(train_res.diffusivities_noNN.Ri[2:end-1, $n], -10, 10)
        ...
        lines!(axT, T_noNNₙ, zC, color=colors[2], linewidth=3, label="Base Closure Only")
        lines!(axS, S_noNNₙ, zC, color=colors[2], linewidth=3, label="Base Closure Only")
        lines!(axσ, ρ_noNNₙ, zC, color=colors[2], linewidth=3, label="Base Closure Only")

        lines!(axT, T_predₙ, zC, color=:black, linewidth=3, label="NODE")
        lines!(axS, S_predₙ, zC, color=:black, linewidth=3, label="NODE")
        lines!(axσ, ρ_predₙ, zC, color=:black, linewidth=3, label="NODE")

        lines!(axwT, wT_truthₙ, zF[1:end-1], color=(colors[1], 0.5), linewidth=5, label="LES Solution")
        lines!(axwS, wS_truthₙ, zF[1:end-1], color=(colors[1], 0.5), linewidth=5, label="LES Solution")
        
        lines!(axwT, wT_noNNₙ, zF[1:end-1], color=colors[2], linewidth=3, label="Base Closure Only, No NN")
        lines!(axwS, wS_noNNₙ, zF[1:end-1], color=colors[2], linewidth=3, label="Base Closure Only, No NN")
        
        lines!(axwT, wT_residualₙ, zF[1:end-1], color=colors[3], linewidth=3, label="NN Contribution")
        lines!(axwS, wS_residualₙ, zF[1:end-1], color=colors[3], linewidth=3, label="NN Contribution")

        lines!(axwT, wT_diffusive_boundaryₙ, zF[1:end-1], color=colors[4], linewidth=3, label="Base Closure Contribution")
        lines!(axwS, wS_diffusive_boundaryₙ, zF[1:end-1], color=colors[4], linewidth=3, label="Base Closure Contribution")

        lines!(axwT, wT_predₙ, zF[1:end-1], color=:black, linewidth=3, label="NODE")
        lines!(axwS, wS_predₙ, zF[1:end-1], color=:black, linewidth=3, label="NODE")

        lines!(axRi, Ri_truthₙ, zF[2:end-1], color=(colors[1], 0.5), linewidth=3)
        lines!(axRi, Ri_noNNₙ, zF[2:end-1], color=colors[2], linewidth=3)
        lines!(axRi, Ri_predₙ, zF[2:end-1], color=:black, linewidth=3)

        lines!(axν, κ_noNNₙ, zF[2:end-1], color=colors[2], linewidth=3, label="Base Closure Only")
        lines!(axν, κ_predₙ, zF[2:end-1], color=:black, linewidth=3, label="NODE")

        linkyaxes!(axT, axS, axσ, axν, axwT, axwS, axRi)
        ...
        xlims!(axwS, wSlim)
        xlims!(axν, νlim)
        xlims!(axRi, (-10.5, 10.5))

        Legend(l1[2, 4], axwT, tellwidth=false)

        # display(fig)
        CairoMakie.record(fig, "./slides_figures/slightlylocalNN_localbaseclosure/training_$(i).mp4", 1:Nt, framerate=20) do nn
            n[] = nn
        end
    end
end
...
for (i, (train_res, truth_train)) in enumerate(zip(interpolating_results, interpolating_data_plot.data))
    ...
    with_theme(theme_latexfonts()) do
        ...
        n = Observable(1)
        time_str = @lift "NN + Local Base Closure Validation: Qᵁ = $(Qᵁ) m² s⁻², Qᵀ = $(Qᵀ) m s⁻¹ °C, Qˢ = $(Qˢ) m s⁻¹ g kg⁻¹, f = $(f) s⁻¹, Time = $(round(times[$n], digits=1)) days"
        ...
        # display(fig)
        CairoMakie.record(fig, "./slides_figures/slightlylocalNN_localbaseclosure/validation_$(i).mp4", 1:Nt, framerate=20) do nn
            n[] = nn
        end
    end
end
#%%
for (i, (train_res, truth_train, loss, loss_noNN)) in enumerate(zip(training_results, training_data_plot.data, weighted_training_loss, weighted_training_loss_noNN))
    @info "Plotting $i"
    ...
    with_theme(theme_latexfonts()) do
        ...
        axwT = CairoMakie.Axis(l1[2,1], ylabel="z (m)", xlabel=L"$\overline{w\prime T\prime}$ (°C m s$^{-1}$)", xticks=LinearTicks(2))
        axwS = CairoMakie.Axis(l1[2,2], ylabel="z (m)", xlabel=L"$\overline{w\prime S\prime}$ (g kg$^{-1}$ m s$^{-1}$)", xticks=LinearTicks(2))
        axloss = CairoMakie.Axis(l1[2,3], xlabel="Time (days)", ylabel="Loss", yscale=log10)
        ...
        wT_residualₙ = @lift train_res.fluxes.wT.residual[2:end-2, $n]
        wS_residualₙ = @lift train_res.fluxes.wS.residual[2:end-2, $n]
        ...
        lines!(axwT, wT_diffusive_boundaryₙ, zF[1:end-1], color=colors[2], linewidth=3, label="Base Closure Only")
        lines!(axwS, wS_diffusive_boundaryₙ, zF[1:end-1], color=colors[2], linewidth=3, label="Base Closure Only")
        ...
        lines!(axwT, wT_residualₙ, zF[2:end-2], color=colors[3], linewidth=3, label="NN Contribution")
        lines!(axwS, wS_residualₙ, zF[2:end-2], color=colors[3], linewidth=3, label="NN Contribution")

        lines!(axloss, times, loss, linewidth=3, color=:black, label="NODE")
        lines!(axloss, times, loss_noNN, linewidth=3, color=colors[2], label="Base Closure Only")
        vlines!(axloss, timeₙ, color=colors[5], linewidth=3)

        linkyaxes!(axT, axS, axσ, axwT, axwS)
        ...
        Legend(l1[3, :], axwT, tellwidth=false, orientation=:horizontal)

        # display(fig)
        CairoMakie.record(fig, "./slides_figures/slightlylocalNN_localbaseclosure_simple/training_$(i).mp4", 1:Nt, framerate=20) do nn
                n[] = nn
        end
    end
end
#%%
for (i, (train_res, truth_train, loss, loss_noNN)) in enumerate(zip(interpolating_results, interpolating_data_plot.data, weighted_interpolating_loss, weighted_interpolating_loss_noNN))
    @info "Plotting $i"
    ...
    with_theme(theme_latexfonts()) do
        ...
        # display(fig)
        CairoMakie.record(fig, "./slides_figures/slightlylocalNN_localbaseclosure_simple/validation_$(i).mp4", 1:Nt, framerate=20) do nn
                n[] = nn
        end
    end
end
