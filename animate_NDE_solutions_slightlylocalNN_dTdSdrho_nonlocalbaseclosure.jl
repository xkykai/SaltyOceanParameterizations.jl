...
for (i, (train_res, truth_train)) in enumerate(zip(training_results, training_data_plot.data))
    ...
    with_theme(theme_latexfonts()) do
        ...
        n = Observable(1)
        time_str = @lift "NN + Nonlocal Base Closure Training: Qᵁ = $(Qᵁ) m² s⁻², Qᵀ = $(Qᵀ) m s⁻¹ °C, Qˢ = $(Qˢ) m s⁻¹ g kg⁻¹, f = $(f) s⁻¹, Time = $(round(times[$n], digits=1)) days"
        ...
        # display(fig)
        CairoMakie.record(fig, "./slides_figures/slightlylocalNN_nonlocalbaseclosure/training_$(i).mp4", 1:Nt, framerate=20) do nn
            n[] = nn
        end
    end
end
...
for (i, (train_res, truth_train)) in enumerate(zip(interpolating_results, interpolating_data_plot.data))
    ...
    with_theme(theme_latexfonts()) do
        ...
        n = Observable(1)
        time_str = @lift "NN + Nonlocal Base Closure Validation: Qᵁ = $(Qᵁ) m² s⁻², Qᵀ = $(Qᵀ) m s⁻¹ °C, Qˢ = $(Qˢ) m s⁻¹ g kg⁻¹, f = $(f) s⁻¹, Time = $(round(times[$n], digits=1)) days"

        Label(fig[0, :], time_str, font=:bold, fontsize=18)
        ...
        # display(fig)
        CairoMakie.record(fig, "./slides_figures/slightlylocalNN_nonlocalbaseclosure/validation_$(i).mp4", 1:Nt, framerate=20) do nn
            n[] = nn
        end
    end
end
