...
with_theme(theme_latexfonts()) do
    ...
    axwT = CairoMakie.Axis(fig[2,1], ylabel="z (m)", xlabel=L"$\overline{w \prime T \prime}$ (m s$^{-1}$ °C)")
    ...
    Label(fig[0, :], "Neural Networks only Training: Strong Cooling, 5°S - 5°N Pacific JJA", font=:bold)

    res = training_results[5]
    truth = training_data_plot.data[5]
    loss = weighted_training_losses[5]
    ...
    display(fig)
    # save("./figures/slightlylocalNN_training_5.png", fig, px_per_unit=8)
end
...
with_theme(theme_latexfonts()) do
    ...
    axT = CairoMakie.Axis(fig[1,1], ylabel="z (m)", xlabel=L"$\overline{T}$ (°C)")
    axS = CairoMakie.Axis(fig[1,2], ylabel="z (m)", xlabel=L"$\overline{S}$ (g kg$^{-1}$)")
    axσ = CairoMakie.Axis(fig[1,3], ylabel="z (m)", xlabel=L"$\overline{\sigma}$ (kg m$^{-3}$)", xticks=LinearTicks(2))
    ...
    Label(fig[0, :], "Neural Networks only Training: Strong Evaporation, 30°N - 40°N Atlantic JJA", font=:bold)

    res = training_results[3]
    truth = training_data_plot.data[3]
    loss = weighted_training_losses[3]
    ...
    display(fig)
    # save("./figures/slightlylocalNN_training_3.png", fig, px_per_unit=8)
end
...
with_theme(theme_latexfonts()) do
    ...
    Label(fig[0, :], "Neural Networks only Validation: Cooling + Evaporation, 30°N - 40°N Atlantic JJA", font=:bold)
    ...
    display(fig)
    # save("./figures/slightlylocalNN_interpolation_10.png", fig, px_per_unit=8)
end
#%%
with_theme(theme_latexfonts()) do
    fig = Figure(size=(1200, 775), fontsize=20)

    axT = CairoMakie.Axis(fig[1,1], ylabel="z (m)", xlabel=L"$\overline{T}$ (°C)", xticks=LinearTicks(3))
    axS = CairoMakie.Axis(fig[1,2], ylabel="z (m)", xlabel=L"$\overline{S}$ (g kg$^{-1}$)", xticks=LinearTicks(3))
    axσ = CairoMakie.Axis(fig[1,3], ylabel="z (m)", xlabel=L"$\overline{\sigma}$ (kg m$^{-3}$)", xticks=LinearTicks(3))
    ...
    Label(fig[0, :], "Neural Networks only Validation: Cooling + Evaporation, 5°S - 5°N Pacific JJA", font=:bold)
    ...
    xlims!(axT, 11, 14.5)
    xlims!(axS, 34.8, 35.02)
    xlims!(axσ, 1025.95, 1026.5)

    xlims!(axwT, -2e-4, 2e-4)
    xlims!(axwS, -6e-5, 8e-5)

    display(fig)
    # save("./figures/slightlylocalNN_interpolation_9.png", fig, px_per_unit=8)
end
#%%
with_theme(theme_latexfonts()) do
    fig = Figure(size=(1200, 775), fontsize=20)
    ...
    Label(fig[0, :], "Neural Networks only Validation: Strong Cooling, 30°N - 40°N Atlantic JJA", font=:bold)
    ...
    display(fig)
    # save("./figures/slightlylocalNN_interpolation_1.png", fig, px_per_unit=8)
end
#%%