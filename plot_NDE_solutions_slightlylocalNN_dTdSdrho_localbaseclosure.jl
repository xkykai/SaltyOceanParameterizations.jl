...
with_theme(theme_latexfonts()) do
    ...
    axwT = CairoMakie.Axis(fig[2,1], ylabel="z (m)", xlabel=L"$\overline{w \prime T \prime}$ (m s$^{-1}$ °C)", xticks=LinearTicks(2))
    ...
    Label(fig[0, :], "Neural Networks + Local Base Closure Training: Strong Cooling, 30°N - 40°N Atlantic JJA", font=:bold)
    ...
    for (i, n) in enumerate(timeframes)
        ...
        if i == 1
            ...
            lines!(axS, res.sols_dimensional_noNN.S[:, n], zC, color=colors[i], linestyle=:dot, linewidth=2, label="Local Base Closure only")
        else
    ...
    xlims!(axwT, nothing, 0.0006)

    display(fig)
    # save("./figures/slightlylocalNN_localbaseclosure_training_1.png", fig, px_per_unit=8)
end
...
with_theme(theme_latexfonts()) do
    ...
    Label(fig[0, :], "Neural Networks + Local Base Closure Training: Strong Evaporation, 5°S - 5°N Pacific JJA", font=:bold)
    ...
    xlims!(axwT, -2e-4, nothing)
    xlims!(axwS, -6e-5, nothing)

    display(fig)
    # save("./figures/slightlylocalNN_localbaseclosure_training_7.png", fig, px_per_unit=8)
end
...
with_theme(theme_latexfonts()) do
    ...
    xlims!(axwT, nothing, 0.0004)
    xlims!(axwS, -5e-5, nothing)

    Legend(fig[3, 1:2], axT, orientation=:horizontal, tellwidth=false)
    Legend(fig[3, 3:4], axS, orientation=:horizontal, tellwidth=false)

    display(fig)
    # save("./figures/slightlylocalNN_localbaseclosure_interpolation_9.png", fig, px_per_unit=8)
end
...
with_theme(theme_latexfonts()) do
    ...
    Label(fig[0, :], "Neural Networks + Local Base Closure Validation: Weak Cooling, 30°N - 40°N Atlantic JJA", font=:bold)

    res = interpolating_results[2]
    truth = interpolating_data_plot.data[2]
    loss = weighted_interpolating_losses[2]
    ...
    display(fig)
    # save("./figures/slightlylocalNN_localbaseclosure_interpolation_2.png", fig, px_per_unit=8)
end
...
with_theme(theme_latexfonts()) do
    ...
    Label(fig[0, :], "Neural Networks + Local Base Closure Validation: Cooling + Evaporation, 30°N - 40°N Atlantic JJA", font=:bold)
    ...
    display(fig)
    # save("./figures/slightlylocalNN_localbaseclosure_interpolation_10.png", fig, px_per_unit=8)
end
#%%