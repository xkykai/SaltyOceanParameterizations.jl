...
training_results = jldopen("./plots_data/slightlylocalNN_localbaseclosure_dTdSdrho_training_results.jld2")["u"]
interpolating_results = jldopen("./plots_data/slightlylocalNN_localbaseclosure_dTdSdrho_interpolating_results.jld2")["u"]
...
with_theme(theme_latexfonts()) do
    ...
    axS = CairoMakie.Axis(l1[1,2], ylabel="z (m)", xlabel=L"$\overline{S}$ (g kg$^{-1}$)", xticks=LinearTicks(3))
    axT_NN = CairoMakie.Axis(l1[1,3], ylabel="z (m)", xlabel=L"$\overline{T}$ (°C)")
    axS_NN = CairoMakie.Axis(l1[1,4], ylabel="z (m)", xlabel=L"$\overline{S}$ (g kg$^{-1}$)", xticks=LinearTicks(3))

    Label(l1[0, 1:2], "Physical Closure Only", font=:bold, fontsize=30)
    Label(l1[0, 3:4], "Neural Networks + Physical Closure", font=:bold, fontsize=30)

    # Label(fig[0, :], "Neural Networks + Local Base Closure Validation: Cooling + Evaporation, 5°S - 5°N Pacific JJA", font=:bold)
    ...
    for (i, n) in enumerate(timeframes)
        lines!(axT, truth.profile.T.unscaled[:, n], zC, color=colors[i], alpha=0.4, linewidth=15, label="$(times[i]) days")
        lines!(axT_NN, truth.profile.T.unscaled[:, n], zC, color=colors[i], alpha=0.4, linewidth=15, label="$(times[i]) days")
        
        if i == 1
            lines!(axS, truth.profile.S.unscaled[:, n], zC, color=(colors[i], 0.4), linewidth=15, label="Large Eddy Simulation")
            lines!(axS, res.sols_dimensional_noNN.S[:, n], zC, color=colors[i], linestyle=:dot, linewidth=5, label="Physical Closure Only")
            lines!(axS_NN, truth.profile.S.unscaled[:, n], zC, color=(colors[i], 0.4), linewidth=15, label="Large Eddy Simulation")
            lines!(axS_NN, res.sols_dimensional.S[:, n], zC, color=colors[i], linestyle=:dash, linewidth=5, label="ML-Augmented Parameterization")
        else
            lines!(axS, truth.profile.S.unscaled[:, n], zC, color=(colors[i], 0.4), linewidth=15)
            lines!(axS, res.sols_dimensional_noNN.S[:, n], zC, color=colors[i], linestyle=:dot, linewidth=5)
            lines!(axS_NN, truth.profile.S.unscaled[:, n], zC, color=(colors[i], 0.4), linewidth=15)
            lines!(axS_NN, res.sols_dimensional.S[:, n], zC, color=colors[i], linestyle=:dash, linewidth=5)
        end
        
        lines!(axT_NN, res.sols_dimensional.T[:, n], zC, color=colors[i], linestyle=:dash, linewidth=5)
        lines!(axT, res.sols_dimensional_noNN.T[:, n], zC, color=colors[i], linestyle=:dot, linewidth=5)
    end
    
    linkyaxes!(axT, axS, axT_NN, axS_NN)

    hidedecorations!(axT, ticks=false, ticklabels=false, label=false)
    hidedecorations!(axS, ticks=false, ticklabels=false, label=false)
    hidedecorations!(axT_NN, ticks=false, ticklabels=false, label=false)
    hidedecorations!(axS_NN, ticks=false, ticklabels=false, label=false)

    hideydecorations!(axT, ticks=false, ticklabels=false, label=false)
    hideydecorations!(axS, ticks=false)
    hideydecorations!(axT_NN, ticks=false)
    hideydecorations!(axS_NN, ticks=false)

    hidexdecorations!(axT, ticks=false, ticklabels=false, label=false)
    hidexdecorations!(axT_NN, ticks=false, ticklabels=false, label=false)
    hidexdecorations!(axS, ticks=false, ticklabels=false, label=false)
    hidexdecorations!(axS_NN, ticks=false, ticklabels=false, label=false)

    # Legend(l2[1, 1], axT, orientation=:horizontal, tellwidth=false, labelfont=:bold)
    axislegend(axT, position=:lt, labelfont=:bold)

    lin_LES = LineElement(color=(:black, 0.4), linewidth=15)
    lin_base = LineElement(color=:black, linewidth=5, linestyle=:dot)
    lin_NN = LineElement(color=:black, linewidth=5, linestyle=:dash)

    # Legend(l2[1, 1], axS, orientation=:horizontal, tellwidth=false, labelfont=:bold, patchsize=(70, 20), labelsize=21)
    Legend(l2[1, 1], [lin_LES, lin_base, lin_NN], ["Large Eddy Simulation", "Physical Closure Only", "NN + Physical Closure"], orientation=:horizontal, tellwidth=false, labelfont=:bold, patchsize=(50, 20), labelsize=30)

    trim!(fig.layout)

    display(fig)
    save("./poster_figures/slightlylocalNN_localbaseclosure_interpolation_9_2.png", fig, px_per_unit=16)
end
...
with_theme(theme_latexfonts()) do
    ...
    axσ = CairoMakie.Axis(l1[1,3], ylabel="z (m)", xlabel=L"$\overline{\sigma}$ (kg m$^{-3}$)", xticks=LinearTicks(3))
    ...
    for (i, n) in enumerate(timeframes)
        lines!(axT, truth.profile.T.unscaled[:, n], zC, color=colors[i], alpha=0.5, linewidth=10, label="$(times[i]) days")
        lines!(axσ, truth.profile.ρ.unscaled[:, n], zC, color=(colors[i], 0.5), linewidth=10, label="$(times[i]) days")
        
        if i == 1
            lines!(axS, truth.profile.S.unscaled[:, n], zC, color=(colors[i], 0.5), linewidth=10, label="Large Eddy Simulation")
            lines!(axS, res.sols_dimensional.S[:, n], zC, color=:black, linewidth=5, label="ML-Augmented Parameterization")
            lines!(axS, res.sols_dimensional_noNN.S[:, n], zC, color=colors[i], linestyle=:dash, linewidth=5, label="Physical Closure Only")
        else
            lines!(axS, truth.profile.S.unscaled[:, n], zC, color=(colors[i], 0.5), linewidth=10)
            lines!(axS, res.sols_dimensional.S[:, n], zC, color=:black, linewidth=5)
            lines!(axS, res.sols_dimensional_noNN.S[:, n], zC, color=colors[i], linestyle=:dash, linewidth=5)
        end
        
        lines!(axT, res.sols_dimensional.T[:, n], zC, color=:black, linewidth=5)
        lines!(axσ, res.sols_dimensional.ρ[:, n], zC, color=:black, linewidth=5)

        lines!(axT, res.sols_dimensional_noNN.T[:, n], zC, color=colors[i], linestyle=:dash, linewidth=5)
        lines!(axσ, res.sols_dimensional_noNN.ρ[:, n], zC, color=colors[i], linestyle=:dash, linewidth=5)
    ...
    display(fig)
    # save("./poster_figures/slightlylocalNN_localbaseclosure_interpolation_9.png", fig, px_per_unit=16)
end

with_theme(theme_latexfonts()) do
    fig = Figure(size=(700, 700), fontsize=30)

    axwT = CairoMakie.Axis(fig[1,1], ylabel="z (m)", xlabel=L"$\overline{w \prime T \prime}$ (m s$^{-1}$ °C)", xticks=LinearTicks(3))
    ...
    lines!(axwT, truth.flux.wT.column.unscaled[1:end-1, timeframe], zF[1:end-1], color=(colors[3], 0.4), linewidth=15, label="Large Eddy Simulation")
    lines!(axwT, res.fluxes.wT.diffusive_boundary[1:end-2, timeframe], zF[1:end-2], color=colors[2], linewidth=5, linestyle=:dot, label="Physical Closure")
    lines!(axwT, res.fluxes.wT.total[1:end-1, timeframe], zF[1:end-1], color=:black, linestyle=:dash, linewidth=5, label="NN + Physical Closure")
    # lines!(axwT, res.fluxes.wT.residual[1:end-2, timeframe], zF[1:end-2], color=colors[2], linewidth=5, linestyle=:dash, label="Neural Network")
    
    hidedecorations!(axwT, ticks=false, ticklabels=false, label=false)

    axislegend(axwT, position=:lt, labelfont=:bold, patchsize=(50, 20), labelsize=24)
    # Legend(fig[3, 1:2], axT, orientation=:horizontal, tellwidth=false)
    # Legend(fig[3, 3:4], axS, orientation=:horizontal, tellwidth=false)

    display(fig)
    # save("./poster_figures/wT_training_1.png", fig, px_per_unit=16)
end