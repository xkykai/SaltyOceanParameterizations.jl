...
with_theme(theme_latexfonts()) do
    ...
    lines!(ax, Ris, νs, label="Viscosity ν")
    lines!(ax, Ris, νs ./ ps_baseclosure.Pr, label="Diffusivity κ")
    axislegend(ax, position=:rt)
    hidedecorations!(ax, ticks=false, ticklabels=false, label=false)
    vlines!(ax, [0, Ric], color=:black, linestyle=:dash, linewidth=2)
    text!(ax, -0.3, 0.25, text="Convection-driven mixing", align=(:center, :center))
    text!(ax, Ric/2, 0.25, text="Shear-driven mixing", align=(:center, :center))

    display(fig)
    save("./figures/localbaseclosure_.png", fig, px_per_unit=8)
end
#%%