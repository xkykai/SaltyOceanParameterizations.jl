...
function calculate_NDE(train_data, train_data_plot, NN, ps_NN, st_NN; coarse_size=32, dev=cpu_device(), Ri_clamp_lims=(-10, 10), solver=ROCK2())
    ...
    function predict_diffusivities(Ris, p, st)
        ...
        νs = [diffusivity[1] / 100 + ν₀ for diffusivity in diffusivities]
        κs = [diffusivity[2] / 100 + κ₀ for diffusivity in diffusivities]
    ...
    sols_posttraining = predict_NDE_posttraining(ps_NN)
    ...
    for (i, sol) in enumerate(sols_posttraining)
        for t in eachindex(sol.t)
            uw, vw, wT, wS = predict_total_flux_dimensional(sol[:, t], ps_NN, params[i], st_NN)
            ...
            Ris_truth = calculate_Ri(train_data_plot.data[i].profile.u.unscaled[:, t], 
                                     ...
                                     params[i].Dᶠ, params[i].g, eos.reference_density, clamp_lims=Ri_clamp_lims)
            νs, κs = predict_diffusivities(Ris, ps_NN, st_NN)
...
epoch = "divide10"
sols, fluxes, diffusivities = calculate_NDE(train_data, train_data_plot, baseclosure_NN, ps_baseclosure, st_baseclosure, Ri_clamp_lims=(-Inf, Inf), solver=ROCK2())
