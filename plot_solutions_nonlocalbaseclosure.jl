...
with_theme(theme_latexfonts()) do
    ...
    Label(fig[0, :], "Nonlocal Base Closure Training: Weak Wind + Strong Cooling, 30°N - 40°N Atlantic JJA", font=:bold)

    train_res = training_results[7]
    truth_train = training_data_plot.data[7]
    ...
    for (i, n) in enumerate(timeframes)
        ...
        if i == 1
            ...
            lines!(axv, train_res.sols_dimensional.v[:, n], zC, color=colors[i], linestyle=:dot, linewidth=2, label="Nonlocal Base Closure")
        else
    ...
    xlims!(axuw, -2e-4, nothing)
    xlims!(axvw, nothing, 1e-4)
    xlims!(axwT, -4e-4, 5e-4)
    ...
    display(fig)
    # save("./figures/nonlocalbaseclosure_convectivetanh_shearlinear_training_7.png", fig, px_per_unit=8)
end
#%%
with_theme(theme_latexfonts()) do
    fig = Figure(size=(1400, 700), fontsize=20)
    ...
    Label(fig[0, :], "Nonlocal Base Closure Training: Weak Wind + Strong Evaporation, 5°S - 5°N Pacific JJA", font=:bold)

    train_res = training_results[19]
    truth_train = training_data_plot.data[19]
    ...
    display(fig)
    # save("./figures/nonlocalbaseclosure_convectivetanh_shearlinear_training_19.png", fig, px_per_unit=8)
end
#%%
with_theme(theme_latexfonts()) do
    fig = Figure(size=(1400, 700), fontsize=20)
    ...
    Label(fig[0, :], "Nonlocal Base Closure Validation: Weak Wind + Strong Cooling, 30°N - 40°N Atlantic JJA", font=:bold)

    train_res = interpolating_results[13]
    truth_train = interpolating_data_plot.data[13]
    ...
    display(fig)
    # save("./figures/nonlocalbaseclosure_convectivetanh_shearlinear_interpolation_13.png", fig, px_per_unit=8)
end
#%%
with_theme(theme_latexfonts()) do
    fig = Figure(size=(1400, 700), fontsize=20)
    ...
    Label(fig[0, :], "Nonlocal Base Closure Validation: Strong Wind + Weak Cooling, 30°N - 40°N Atlantic JJA", font=:bold)

    train_res = interpolating_results[22]
    truth_train = interpolating_data_plot.data[22]
    ...
    xlims!(axuw, -4e-4, 2e-4)
    xlims!(axvw, -6e-5, 1e-4)
    ...
    display(fig)
    # save("./figures/nonlocalbaseclosure_convectivetanh_shearlinear_interpolation_22.png", fig, px_per_unit=8)
end
#%%