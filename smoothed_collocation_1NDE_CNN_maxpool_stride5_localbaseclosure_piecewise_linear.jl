...
FILE_DIR = "./training_output/CNN_maxpool_stride5_128_swish_local_diffusivity_piecewise_linear_noclamp_smoothed_collocation"
mkpath(FILE_DIR)
@info "$(FILE_DIR)"
...
BASECLOSURE_FILE_DIR = "./training_output/local_diffusivity_piecewise_linear_noclamp_lossequal_reltol1e-5/training_results_2.jld2"
...
function train_NDE(train_data, train_data_plot, NNs, ps_training, ps_baseclosure, st_NN, rng; 
                   coarse_size=32, dev=cpu_device(), maxiter=10, optimizer=OptimizationOptimisers.ADAM(0.001), solver=ROCK2(), Ri_clamp_lims=(-Inf, Inf), kernel=EpanechnikovKernel())
    ...
    profile_collocations = [zeros(coarse_size*4, length(param.scaled_original_time)) for param in params]
    profile_dt_collocations = [zeros(coarse_size*4, length(param.scaled_original_time)) for param in params]
    
    for (data, profile, profile_dt, param) in zip(train_data_plot.data, profile_collocations, profile_dt_collocations, params)
        tsteps = param.scaled_original_time

        du, u = collocate_data(vcat(data.profile.u.scaled, data.profile.v.scaled, data.profile.T.scaled, data.profile.S.scaled), tsteps, kernel)

        profile .= u
        profile_dt .= du
    end

    function predict_RHS(p)
        NDE_RHSs = [hcat([NDE(profile[:, i], p, 0, param, st_NN) for i in axes(profile, 2)]...) for (profile, param) in zip(profile_collocations, params)]
        return NDE_RHSs
    end

    function predict_losses(RHSs, loss_scaling=(; u=1, v=1, T=1, S=1))
        ses = [(RHS .- profile_dt) .^2 for (RHS, profile_dt) in zip(RHSs, profile_dt_collocations)]
        u_loss = loss_scaling.u * mean(mean([@view(se[1:coarse_size, :]) for se in ses]))
        v_loss = loss_scaling.v * mean(mean([@view(se[coarse_size+1:2*coarse_size, :]) for se in ses]))
        T_loss = loss_scaling.T * mean(mean([@view(se[2*coarse_size+1:3*coarse_size, :]) for se in ses]))
        S_loss = loss_scaling.S * mean(mean([@view(se[3*coarse_size+1:4*coarse_size, :]) for se in ses]))

        return (u=u_loss, v=v_loss, T=T_loss, S=S_loss)
    end

    function compute_loss_prefactor(u_loss, v_loss, T_loss, S_loss)
        T_prefactor = 1
    ...
    function compute_loss_prefactor(p)
        losses = predict_losses(predict_RHS(p))
    ...
    function loss_RHS(p)
        preds = predict_RHS(p)
        individual_loss = predict_losses(preds, losses_prefactor)
    ...
    callback = function (p, l, pred, ind_loss)
        @printf("%s, Δt %s, iter %d/%d, loss total %6.10e, u %6.5e, v %6.5e, T %6.5e, S %6.5e,\n",
    ...
    optf = Optimization.OptimizationFunction((x, p) -> loss_RHS(x), adtype)
    ...
    losses = (total=losses, u=u_losses, v=v_losses, T=T_losses, S=S_losses)
    collocations = (profile=profile_collocations, profile_dt=profile_dt_collocations)

    return res, loss_RHS(res.u), sols_posttraining, flux_posttraining, losses, diffusivities_posttraining, collocations
end
...
res, loss, sols, fluxes, losses, diffusivities, collocations = train_NDE(train_data, train_data_plot, NNs, ps_training, ps_baseclosure, st_NN, rng, maxiter=1000, solver=VCABM3(), optimizer=OptimizationOptimisers.ADAM(1e-2))

u = res.u
jldsave("$(FILE_DIR)/training_results_$(epoch).jld2"; res, u, loss, sols, fluxes, losses, NNs, st_NN, diffusivities, collocations)
...
res, loss, sols, fluxes, losses, diffusivities, collocations = train_NDE(train_data, train_data_plot, NNs, res.u, ps_baseclosure, st_NN, rng, maxiter=1000, solver=VCABM3(), optimizer=OptimizationOptimisers.ADAM(2e-3))
u = res.u
jldsave("$(FILE_DIR)/training_results_$(epoch).jld2"; res, u, loss, sols, fluxes, losses, NNs, st_NN, diffusivities, collocations)
plot_loss(losses, FILE_DIR, epoch=epoch)
for i in eachindex(field_datasets)
...
res, loss, sols, fluxes, losses, diffusivities, collocations = train_NDE(train_data, train_data_plot, NNs, res.u, ps_baseclosure, st_NN, rng, maxiter=1000, solver=VCABM3(), optimizer=OptimizationOptimisers.ADAM(1e-3))
...
res, loss, sols, fluxes, losses, diffusivities, collocations = train_NDE(train_data, train_data_plot, NNs, res.u, ps_baseclosure, st_NN, rng, maxiter=1000, solver=VCABM3(), optimizer=OptimizationOptimisers.ADAM(5e-4))
