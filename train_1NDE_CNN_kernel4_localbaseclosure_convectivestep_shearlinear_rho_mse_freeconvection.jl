...
FILE_DIR = "./training_output/CNN_kernel4_4channels_relu_local_diffusivity_convectivetanh_shearlinear_glorot_freeconvection_mse_Adam1e-3"
mkpath(FILE_DIR)
@info "$(FILE_DIR)"
...
CNN = Chain(Conv(Tuple(4), 1 => 4, relu),
            FlattenLayer())

decoder = Chain(Dense(118, 256, relu),
                Dense(256, 256, relu),
                Dense(256, 31))
