...
FILE_DIR = "./training_output/1CNN_128_swish_3_local_diffusivity_clamp_-20_20_fast_VCABM3_ADAM1e-4_rho0.3_test"
mkpath(FILE_DIR)
@info "$(FILE_DIR)"
...
BASECLOSURE_FILE_DIR = "./training_output/local_diffusivity_clamp_-20_20_fast/training_results_1.jld2"
...
NN = Chain(Conv(Tuple(3), 10 => 8, swish),
           Conv(Tuple(3), 8 => 8, swish),
           FlattenLayer(),
        #    Dense(28*8, 124)
           Dense(28*8, 128, swish),
           Dense(128, 124)
)
        #    FlattenLayer(),
        #    Dense(6*16, 128, swish),
...
function train_NDE(train_data, train_data_plot, NNs, ps_training, ps_baseclosure, st_NN, rng; 
    ...
    function predict_diffusivities(Ris, p)
        diffusivities = [local_Ri_diffusivity(Ri, ps_baseclosure.ν₁, ps_baseclosure.Riᶜ, ps_baseclosure.ΔRi, ps_baseclosure.Pr) for Ri in Ris]
    ...
    function predict_NDE(p)
        probs = [ODEProblem((x, p′, t) -> NDE(x, p′, t, param, st_NN), x₀, (param.scaled_time[1], param.scaled_time[end]), p) for (x₀, param) in zip([x₀s[3]], [params[3]])]
        sols = [Array(solve(prob, solver, saveat=param.scaled_time, reltol=1e-3)) for (param, prob) in zip([params[3]], probs)]
        return sols
    end

    function predict_NDE_posttraining(p)
        probs = [ODEProblem((x, p′, t) -> NDE(x, p′, t, param, st_NN), x₀, (param.scaled_original_time[1], param.scaled_original_time[end]), p) for (x₀, param) in zip([x₀s[3]], [params[3]])]
        sols = [solve(prob, solver, saveat=param.scaled_original_time, reltol=1e-3) for (param, prob) in zip([params[3]], probs)]
        return sols
    end

    function predict_scaled_profiles(p)
        preds = predict_NDE(p)
        u_hats = [get_u(pred, param) for (pred, param) in zip(preds, [params[3]])]
        v_hats = [get_v(pred, param) for (pred, param) in zip(preds, [params[3]])]
        T_hats = [get_T(pred, param) for (pred, param) in zip(preds, [params[3]])]
        S_hats = [get_S(pred, param) for (pred, param) in zip(preds, [params[3]])]
        
        us = [inv(param.scaling.u).(u_hat) for (u_hat, param) in zip(u_hats, [params[3]])]
        vs = [inv(param.scaling.v).(v_hat) for (v_hat, param) in zip(v_hats, [params[3]])]
        Ts = [inv(param.scaling.T).(T_hat) for (T_hat, param) in zip(T_hats, [params[3]])]
        Ss = [inv(param.scaling.S).(S_hat) for (S_hat, param) in zip(S_hats, [params[3]])]

        ρs = [calculate_unscaled_density(T, S) for (T, S) in zip(Ts, Ss)]
        ρ_hats = [param.scaling.ρ.(ρ) for (ρ, param) in zip(ρs, [params[3]])]

        ∂u∂z_hats = [hcat([param.scaling.∂u∂z.(param.Dᶠ * @view(u[:, i])) for i in axes(u, 2)]...) for (param, u) in zip([params[3]], us)]
        ∂v∂z_hats = [hcat([param.scaling.∂v∂z.(param.Dᶠ * @view(v[:, i])) for i in axes(v, 2)]...) for (param, v) in zip([params[3]], vs)]
        ∂T∂z_hats = [hcat([param.scaling.∂T∂z.(param.Dᶠ * @view(T[:, i])) for i in axes(T, 2)]...) for (param, T) in zip([params[3]], Ts)]
        ∂S∂z_hats = [hcat([param.scaling.∂S∂z.(param.Dᶠ * @view(S[:, i])) for i in axes(S, 2)]...) for (param, S) in zip([params[3]], Ss)]
        ∂ρ∂z_hats = [hcat([param.scaling.∂ρ∂z.(param.Dᶠ * @view(ρ[:, i])) for i in axes(ρ, 2)]...) for (param, ρ) in zip([params[3]], ρs)]
    ...
    function predict_losses(u, v, T, S, ρ, ∂u∂z, ∂v∂z, ∂T∂z, ∂S∂z, ∂ρ∂z, loss_scaling=(; u=1, v=1, T=1, S=1, ρ=1, ∂u∂z=1, ∂v∂z=1, ∂T∂z=1, ∂S∂z=1, ∂ρ∂z=1))
        u_loss = loss_scaling.u * mean(mean.([(data.profile.u.scaled .- u).^2 for (data, u) in zip([train_data.data[3]], u)]))
        v_loss = loss_scaling.v * mean(mean.([(data.profile.v.scaled .- v).^2 for (data, v) in zip([train_data.data[3]], v)]))
        T_loss = loss_scaling.T * mean(mean.([(data.profile.T.scaled .- T).^2 for (data, T) in zip([train_data.data[3]], T)]))
        S_loss = loss_scaling.S * mean(mean.([(data.profile.S.scaled .- S).^2 for (data, S) in zip([train_data.data[3]], S)]))
        ρ_loss = loss_scaling.ρ * mean(mean.([(data.profile.ρ.scaled .- ρ).^2 for (data, ρ) in zip([train_data.data[3]], ρ)]))

        ∂u∂z_loss = loss_scaling.∂u∂z * mean(mean.([(data.profile.∂u∂z.scaled .- ∂u∂z).^2 for (data, ∂u∂z) in zip([train_data.data[3]], ∂u∂z)]))
        ∂v∂z_loss = loss_scaling.∂v∂z * mean(mean.([(data.profile.∂v∂z.scaled .- ∂v∂z).^2 for (data, ∂v∂z) in zip([train_data.data[3]], ∂v∂z)]))
        ∂T∂z_loss = loss_scaling.∂T∂z * mean(mean.([(data.profile.∂T∂z.scaled .- ∂T∂z).^2 for (data, ∂T∂z) in zip([train_data.data[3]], ∂T∂z)]))
        ∂S∂z_loss = loss_scaling.∂S∂z * mean(mean.([(data.profile.∂S∂z.scaled .- ∂S∂z).^2 for (data, ∂S∂z) in zip([train_data.data[3]], ∂S∂z)]))
        ∂ρ∂z_loss = loss_scaling.∂ρ∂z * mean(mean.([(data.profile.∂ρ∂z.scaled .- ∂ρ∂z).^2 for (data, ∂ρ∂z) in zip([train_data.data[3]], ∂ρ∂z)]))
    ...
    function compute_loss_prefactor(u_loss, v_loss, T_loss, S_loss, ρ_loss, ∂u∂z_loss, ∂v∂z_loss, ∂T∂z_loss, ∂S∂z_loss, ∂ρ∂z_loss)
        ρ_prefactor = 1
        T_prefactor = ρ_loss / T_loss * (0.3/0.3)
        S_prefactor = ρ_loss / S_loss * (0.3/0.3)
        u_prefactor = ρ_loss / u_loss * (0.05/0.3)
        v_prefactor = ρ_loss / v_loss * (0.05/0.3)

        ∂ρ∂z_prefactor = 1
        ∂T∂z_prefactor = ∂ρ∂z_loss / ∂T∂z_loss * (0.3/0.3)
        ∂S∂z_prefactor = ∂ρ∂z_loss / ∂S∂z_loss * (0.3/0.3)
...
res, loss, sols, fluxes, losses, diffusivities = train_NDE(train_data, train_data_plot, NNs, ps_training, ps_baseclosure, st_NN, rng, maxiter=200, solver=VCABM3(), Ri_clamp_lims=(-20, 20), optimizer=OptimizationOptimisers.ADAM(1e-4))
...
res, loss, sols, fluxes, losses, diffusivities = train_NDE(train_data, train_data_plot, NNs, res.u, ps_baseclosure, st_NN, rng, maxiter=200, solver=VCABM3(), Ri_clamp_lims=(-20, 20), optimizer=OptimizationOptimisers.ADAM(1e-4))
u = res.u
jldsave("$(FILE_DIR)/training_results_$(epoch).jld2"; res, u, loss, sols, fluxes, losses, NNs, st_NN, diffusivities)
plot_loss(losses, FILE_DIR, epoch=epoch)
for i in eachindex(field_datasets)
...
res, loss, sols, fluxes, losses, diffusivities = train_NDE(train_data, train_data_plot, NNs, res.u, ps_baseclosure, st_NN, rng, maxiter=200, solver=VCABM3(), Ri_clamp_lims=(-20, 20), optimizer=OptimizationOptimisers.ADAM(5e-5))
