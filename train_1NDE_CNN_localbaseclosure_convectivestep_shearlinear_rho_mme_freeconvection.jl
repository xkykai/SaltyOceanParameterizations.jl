...
FILE_DIR = "./training_output/CNN_swish_local_diffusivity_convectivestep_shearlinear_glorot_freeconvection_mme_Adam1e-3"
mkpath(FILE_DIR)
@info "$(FILE_DIR)"
...
CNN = Chain(Conv(Tuple(3), 1 => 2, swish),
            Conv(Tuple(5), 2 => 4, swish),
            Conv(Tuple(5), 4 => 8, swish),
            FlattenLayer())

decoder = Chain(Dense(178, 256, swish),
                Dense(256, 256, swish),
                Dense(256, 31))
