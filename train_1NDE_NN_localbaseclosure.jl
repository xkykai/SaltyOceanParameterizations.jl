...
FILE_DIR = "./training_output/1NN_leakyrelu_256_local_diffusivity_NDE_gradient_relu_noclamp_VCABM3_GaussAdjoint_fast_test"
mkpath(FILE_DIR)
...
NN = Chain(Dense(165, 256, leakyrelu), Dense(256, 124))
...
NNs = (NDE=NN, baseclosure=baseclosure_NN)
ps_NN = ComponentArray(NDE=ps, baseclosure=ps_baseclosure)
st_NN = (NDE=st, baseclosure=st_baseclosure)
...
function train_NDE(train_data, train_data_plot, NNs, ps_NN, st_NN; 
    ...
    for (i, sol) in enumerate(sols_posttraining)
        for t in eachindex(sol.t)
            ...
            νs, κs = predict_diffusivities(Ris, res.u, params[i], st_NN)
