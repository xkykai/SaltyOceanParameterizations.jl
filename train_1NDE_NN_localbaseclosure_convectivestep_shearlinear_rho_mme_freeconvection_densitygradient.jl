...
FILE_DIR = "./training_output/NN_densitygradient_swish_local_diffusivity_convectivestep_shearlinear_glorot_freeconvection_mme_Adam1e-3"
mkpath(FILE_DIR)
@info "$(FILE_DIR)"
...
NN = Chain(Dense(35, 128, swish), Dense(128, 128, swish), Dense(128, 31))
...
function train_NDE(train_data, train_data_plot, NNs, ps_training, ps_baseclosure, st_NN, rng; 
    ...
    function predict_residual_flux(∂ρ∂z_hat, p, params, st)
        x′ = vcat(∂ρ∂z_hat, params.wρ.scaled.top, params.f_scaled)
    ...
    function predict_residual_flux_dimensional(∂ρ∂z_hat, p, params, st)
        wρ_hat = predict_residual_flux(∂ρ∂z_hat, p, params, st)
    ...
    function predict_diffusivities(Ris, p)
        νs = local_Ri_ν_convectivestep_shearlinear.(Ris, ps_baseclosure.ν_conv, ps_baseclosure.ν_shear, ps_baseclosure.m)
        κs = local_Ri_κ_convectivestep_shearlinear.(νs, ps_baseclosure.Pr)
        return νs, κs
    end
    ...
    function NDE(x, p, t, params, st)
        ...
        ∂ρ∂z_hat = calculate_scaled_profile_gradients(ρ, params)
        
        wρ_residual = predict_residual_flux(∂ρ∂z_hat, p, params, st)
    ...
    function predict_losses(ρ, ∂ρ∂z, loss_scaling=(; ρ=1, ∂ρ∂z=1))
        ρ_loss = loss_scaling.ρ * mean(maximum.(mean.([(data.profile.ρ.scaled .- ρ).^2 for (data, ρ) in zip(train_data.data, ρ)], dims=1)))
    ...
    for (i, sol) in enumerate(sols_posttraining)
        for t in eachindex(sol.t)
            ...
            ∂ρ∂z_hat = calculate_scaled_profile_gradients(ρ, params[i])

            wρ_diffusive_boundary = predict_diffusive_boundary_flux_dimensional(ρ, ρ_hat, res.u, params[i], st_NN)
            wρ_residual = predict_residual_flux_dimensional(∂ρ∂z_hat, res.u, params[i], st_NN)
...
optimizers = [Optimizer(initial=OptimizationOptimisers.Adam(1e-3), initial_learning_rate=1e-3, learning_rate=1e-3, warmup=1, maxiter=500),
              Optimizer(initial=OptimizationOptimisers.Adam(1e-6), initial_learning_rate=1e-6, learning_rate=5e-4, warmup=20, maxiter=500),]
