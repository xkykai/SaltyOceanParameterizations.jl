...
FILE_DIR = "./training_output/NN_relu_2layer_128_local_diffusivity_convectivestep_shearlinear_glorot_freeconvection_mse_Adam1e-3_nogradient"
mkpath(FILE_DIR)
@info "$(FILE_DIR)"
...
NN = Chain(Dense(34, 128, relu), Dense(128, 128, relu), Dense(128, 31))
# NN = Chain(Dense(34, 4, swish), Dense(4, 4, swish), Dense(4, 31))
...
function train_NDE(train_data, train_data_plot, NNs, ps_training, ps_baseclosure, st_NN, rng; 
    ...
    function predict_losses(ρ, loss_scaling=(; ρ=1, ∂ρ∂z=1))
        ρ_loss = loss_scaling.ρ * mean(mean.([(data.profile.ρ.scaled .- ρ).^2 for (data, ρ) in zip(train_data.data, ρ)]))
        return (; ρ=ρ_loss)
    end

    function loss_NDE(p)
        ρ, preds = predict_scaled_profiles(p)
        individual_loss = predict_losses(ρ)
    ...
    callback = function (p, l, pred, ind_loss)
        @printf("%s, Δt %s, iter %d/%d, loss total %6.10e, ρ %6.5e, max NN weight %6.5e\n",
    ...
    losses = (total=losses, ρ=ρ_losses)
