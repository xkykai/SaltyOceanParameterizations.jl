...
FILE_DIR = "./training_output/NN_relu_2layer_256_local_diffusivity_convectivetanh_shearlinear_glorot_freeconvection_mse_profilegradientinput"
mkpath(FILE_DIR)
@info "$(FILE_DIR)"
...
NN = Chain(Dense(67, 256, relu), Dense(256, 256, relu), Dense(256, 31))
# NN = Chain(Dense(34, 4, relu), Dense(4, 4, relu), Dense(4, 31))
...
function train_NDE(train_data, train_data_plot, NNs, ps_training, ps_baseclosure, st_NN, rng; 
    ...
    function predict_residual_flux(ρ_hat, ∂ρ∂z_hat, p, params, st)
    ...
    function predict_residual_flux_dimensional(ρ_hat, ∂ρ∂z_hat, p, params, st)
        wρ_hat = predict_residual_flux(ρ_hat, ∂ρ∂z_hat,  p, params, st)
    ...
    function NDE(x, p, t, params, st)
        ...
        wρ_residual = predict_residual_flux(ρ_hat, ∂ρ∂z_hat, p, params, st)
    ...
    for (i, sol) in enumerate(sols_posttraining)
        for t in eachindex(sol.t)
            ...
            wρ_residual = predict_residual_flux_dimensional(ρ_hat, ∂ρ∂z_hat, res.u, params[i], st_NN)
