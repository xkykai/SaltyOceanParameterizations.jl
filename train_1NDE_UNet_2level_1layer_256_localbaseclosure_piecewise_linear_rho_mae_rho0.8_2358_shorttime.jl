...
# FILE_DIR = "./training_output/SW_FC_2358_UNet_2level_1layer_256_swish_local_diffusivity_piecewise_linear_rho_rho0.8_mae_Adam_noclamp_rho0.8_mae_ADAM5e-4_test_shorttime"
FILE_DIR = "./training_output/SW_FC_2358_UNet_2level_1layer_256_swish_local_diffusivity_piecewise_linear_rho_rho0.8_gradient_Adam_noclamp_rho0.8_mae_ADAM5e-4_test_shorttime"
mkpath(FILE_DIR)
@info "$(FILE_DIR)"
...
# BASECLOSURE_FILE_DIR = "./training_output/local_diffusivity_piecewise_linear_rho_noclamp_rho0.8_SW_FC_largeinitialdiffusivity_mae/training_results_4.jld2"
...
train_timesteps = [1:10 for data in field_datasets]
...
function train_NDE(train_data, train_data_plot, train_timesteps, NNs, ps_training, ps_baseclosure, st_NN, rng; 
    ...
    params = [(                   f = data.metadata["coriolis_parameter"],
               ...
               scaled_original_time = (plot_data.times .- plot_data.times[1]) ./ (plot_data.times[end] - plot_data.times[1]),
                         train_time = ((data.times .- data.times[1]) ./ (data.times[end] - data.times[1]))[train_timestep],
                                 zC = data.metadata["zC"],
                        ...
                        coarse_size = coarse_size, 
                            ...
                            ) for (data, plot_data, f_feature, uw_feature, vw_feature, wρ_feature, train_timestep) in zip(train_data.data, train_data_plot.data, f_features, uw_features, vw_features, wρ_features, train_timesteps)] |> dev
    ...
    function predict_NDE(p)
        probs = [ODEProblem((x, p′, t) -> NDE(x, p′, t, param, st_NN), x₀, (param.train_time[1], param.train_time[end]), p) for (x₀, param) in zip(x₀s, params)]
        sols = [Array(solve(prob, solver, saveat=param.train_time, reltol=1e-5, sensealg=InterpolatingAdjoint(autojacvec=ZygoteVJP(), checkpointing=true))) for (param, prob) in zip(params, probs)]
        return sols
    end
    ...
    function predict_losses(u, v, ρ, ∂u∂z, ∂v∂z, ∂ρ∂z, loss_scaling=(; u=1, v=1, ρ=1, ∂u∂z=1, ∂v∂z=1, ∂ρ∂z=1))
        u_loss = loss_scaling.u * mean(mean.([abs.(data.profile.u.scaled[:, timestep] .- u′) for (data, u′, timestep) in zip(train_data.data, u, train_timesteps)]))
        v_loss = loss_scaling.v * mean(mean.([abs.(data.profile.v.scaled[:, timestep] .- v′) for (data, v′, timestep) in zip(train_data.data, v, train_timesteps)]))
        ρ_loss = loss_scaling.ρ * mean(mean.([abs.(data.profile.ρ.scaled[:, timestep] .- ρ′) for (data, ρ′, timestep) in zip(train_data.data, ρ, train_timesteps)]))

        ∂u∂z_loss = loss_scaling.∂u∂z * mean(mean.([abs.(data.profile.∂u∂z.scaled[:, timestep] .- ∂u∂z′) for (data, ∂u∂z′, timestep) in zip(train_data.data, ∂u∂z, train_timesteps)]))
        ∂v∂z_loss = loss_scaling.∂v∂z * mean(mean.([abs.(data.profile.∂v∂z.scaled[:, timestep] .- ∂v∂z′) for (data, ∂v∂z′, timestep) in zip(train_data.data, ∂v∂z, train_timesteps)]))
        ∂ρ∂z_loss = loss_scaling.∂ρ∂z * mean(mean.([abs.(data.profile.∂ρ∂z.scaled[:, timestep] .- ∂ρ∂z′) for (data, ∂ρ∂z′, timestep) in zip(train_data.data, ∂ρ∂z, train_timesteps)]))
...
optimizers = [OptimizationOptimisers.ADAM(5e-4),
              ...
              OptimizationOptimisers.ADAM(5e-5)]
