...
FILE_DIR = "./training_output/UNet_large_1level_256_swish_local_diffusivity_piecewise_linear_noclamp_VCABM3_reltol1e-5_ADAM1e-3_glorotuniform_lossequal_mae"
mkpath(FILE_DIR)
@info "$(FILE_DIR)"
...
layer1 = Chain(Conv(Tuple(5), 10 => 16, swish, pad=SamePad()),
               Conv(Tuple(5), 16 => 16, swish, pad=SamePad()),)

layer2 = Chain(MaxPool(Tuple(2), stride=2), 
               Conv(Tuple(5), 16 => 32, swish, pad=SamePad()),
               Upsample(2),
               Conv(Tuple(5), 32 => 16, swish, pad=SamePad()))

layer3 = Chain(Conv(Tuple(5), 32 => 32, swish, pad=SamePad()),
               Conv(Tuple(5), 32 => 16, swish, pad=SamePad()),
               Conv(Tuple(1), 16 => 10, pad=SamePad()),
               FlattenLayer(),)

layer4 = Chain(Dense(32*10, 256, swish),
               Dense(256, 124))
...
res, loss, sols, fluxes, losses, diffusivities = train_NDE(train_data, train_data_plot, NNs, res.u, ps_baseclosure, st_NN, rng, maxiter=200, solver=VCABM3(), optimizer=OptimizationOptimisers.ADAM(2e-4))
