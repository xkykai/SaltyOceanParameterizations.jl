...
uw_NN = Chain(Dense(164, 256, leakyrelu), Dense(256, 31))
vw_NN = Chain(Dense(164, 256, leakyrelu), Dense(256, 31))
wT_NN = Chain(Dense(164, 256, leakyrelu), Dense(256, 31))
wS_NN = Chain(Dense(164, 256, leakyrelu), Dense(256, 31))
as_NN = Chain(Dense(164, 256, leakyrelu), Dense(256, 32))
...
ps_as, st_as = Lux.setup(rng, as_NN)
...
ps_as = ps_as |> ComponentArray .|> Float64
...
NNs = (uw=uw_NN, vw=vw_NN, wT=wT_NN, wS=wS_NN, as=as_NN)
ps_NN = ComponentArray(uw=ps_uw, vw=ps_vw, wT=ps_wT, wS=ps_wS, as=ps_as)
st_NN = (uw=st_uw, vw=st_vw, wT=st_wT, wS=st_wS, as=st_as)

function train_NDE(train_data, NNs, ps_NN, st_NN; coarse_size=32, dev=cpu_device(), maxiter=10, optimizer=OptimizationOptimisers.ADAM(0.01), solver=DP5())
    train_data = train_data |> dev
    x₀s = [vcat(data.profile.u.scaled[:, 1], data.profile.v.scaled[:, 1], data.profile.T.scaled[:, 1], data.profile.S.scaled[:, 1], zeros(coarse_size)) for data in train_data.data] |> dev
    eos = TEOS10EquationOfState()

    uw_NN, vw_NN, wT_NN, wS_NN, as_NN = NNs.uw, NNs.vw, NNs.wT, NNs.wS, NNs.as
    ...
    function predict_NN(x, p, params, st)
        x′ = vcat(x, params.uw.scaled.top, params.vw.scaled.top, params.wT.scaled.top, params.wS.scaled.top)

        uw = vcat(params.uw.scaled.bottom, first(uw_NN(x′, p.uw, st.uw)) .+ params.uw.scaled.bottom, params.uw.scaled.top)
        vw = vcat(params.vw.scaled.bottom, first(vw_NN(x′, p.vw, st.vw)) .+ params.vw.scaled.bottom, params.vw.scaled.top)
        wT = vcat(params.wT.scaled.bottom, first(wT_NN(x′, p.wT, st.wT)) .+ params.wT.scaled.bottom, params.wT.scaled.top)
        wS = vcat(params.wS.scaled.bottom, first(wS_NN(x′, p.wS, st.wS)) .+ params.wS.scaled.bottom, params.wS.scaled.top)
        as = first(as_NN(x′, p.as, st.as))

        return uw, vw, wT, wS, as
    end
    ...
    function NDE(x, p, t, params, st)
        ...
        uw, vw, wT, wS, as = predict_NN(x, p, params, st)

        du = -τ / H * scaling.uw.σ / scaling.u.σ .* (Dᶜ_hat * uw) .+ f * τ ./ scaling.u.σ .* inv(scaling.v).(v)
        dv = -τ / H * scaling.vw.σ / scaling.v.σ .* (Dᶜ_hat * vw) .- f * τ ./ scaling.v.σ .* inv(scaling.u).(u)
        dT = -τ / H * scaling.wT.σ / scaling.T.σ .* (Dᶜ_hat * wT)
        dS = -τ / H * scaling.wS.σ / scaling.S.σ .* (Dᶜ_hat * wS)
        da = as

        return vcat(du, dv, dT, dS, da)
    end
    ...
    for (i, sol) in enumerate(sols_posttraining)
        for t in eachindex(sol.t)
            uw, vw, wT, wS = predict_NN(sol[:, t], res.u, params[i], st_NN)
...
function animate_data(train_data, sols, fluxes, index, FILE_DIR; coarse_size=32, epoch=1)
...
epoch = 1
res, loss, sols, fluxes, losses = train_NDE(train_data, NNs, ps_NN, st_NN, maxiter=500, optimizer=OptimizationOptimisers.ADAM(0.001), solver=DP5())
...
for i in eachindex(field_datasets)
    animate_data(train_data_plot, sols, fluxes, i, FILE_DIR, epoch=epoch)
end

epoch += 1
res, loss, sols, fluxes, losses = train_NDE(train_data, NNs, res.u, st_NN, maxiter=500, optimizer=OptimizationOptimisers.ADAM(0.0002), solver=DP5())
