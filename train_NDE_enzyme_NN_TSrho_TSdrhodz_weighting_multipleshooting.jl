...
function parse_commandline()
    ...
    #     default = "mse"
      ...
      "--S_scaling"
        ...
        default = 1.0
      "--unstable_penalty"
        help = "Penalty for unstable solutions"
        arg_type = Float64
        default = 1.0
    end
    return parse_args(s)
...
const negative_∂ρ∂z_penalty = args["unstable_penalty"]

FILE_DIR = "./training_output/multipleshooting/NDE_enzyme_nobaseclosure_$(args["hidden_layer"])layer_$(args["hidden_layer_size"])_$(args["activation"])_$(S_scaling)Sscaling"
mkpath(FILE_DIR)
@info FILE_DIR
...
sol_T, sol_S, sol_ρ = solve_NDE(ps, params[simulation_indices_split[20]], x₀s_split[20], sts, NNs, params[simulation_indices_split[20]].scaled_Δt, split_length+1)
...
loss(ps, truths_split[20], params[simulation_indices_split[20]], x₀s_split[20], sts, NNs, params[simulation_indices_split[20]].scaled_Δt, length(split_timeframes[20]))
...
ind_losses = [individual_loss(ps, truth, params[sim], x₀, sts, NNs, params[sim].scaled_Δt, length(time)) for (sim, time, truth, x₀) in zip(simulation_indices_split, split_timeframes, truths_split, x₀s_split)]
...
loss_multipleics(ps, [truths_split[20]], [params[simulation_indices_split[20]]], [x₀s_split[20]], sts, NNs, [loss_prefactors[20]], params[simulation_indices_split[20]].scaled_time[1], length(split_timeframes[20]))
...
function train_NDE_multipleics(ps, params, sts, NNs, truths, x₀s, train_data_plot, timeframes, S_scaling, density_contributions; sim_index=[1], epoch=1, maxiter=2, rule=Optimisers.Adam())
    opt_state = Optimisers.setup(rule, ps)
    opt_statemin = deepcopy(opt_state)
    l_min = Inf
    ps_min = deepcopy(ps)
    ...
    losses = zeros(maxiter)
    mean_loss = mean(losses)
    ind_losses = [individual_loss(ps, truth, param, x₀, sts, NNs, param.scaled_Δt, length(timeframe)) for (truth, x₀, param, timeframe) in zip(truths[sim_index], x₀s[sim_index], params[sim_index], timeframes)]
...
for (i, (epoch, optimizer, maxiter)) in enumerate(zip(end_epochs, optimizers, maxiters))
    global ps = ps
    global sols = sols
    ps, losses, opt_state = train_NDE_multipleics(ps, params_split, sts, NNs, truths_split, x₀s_split, train_data_plot, split_timeframes, S_scaling, density_contributions; sim_index=sim_indices, epoch=i, maxiter=maxiter, rule=optimizer)
    
    jldsave("$(FILE_DIR)/training_results_epoch$(epoch)_end$(plot_timeframes[1][end]).jld2"; u=ps, losses=losses, state=opt_state)

    sols = [diagnose_fields(ps, param, x₀, sts, NNs, data, param.scaled_original_time[2] - param.scaled_original_time[1], length(plot_timeframe)) for (data, x₀, param, plot_timeframe) in zip(train_data_plot.data, x₀s, params, plot_timeframes)]
...
for (i, (epoch, optimizer, maxiter, training_timeframe, plot_timeframe)) in enumerate(zip(end_epochs, optimizers, maxiters, training_timeframes, plot_timeframes))
    ...
    ps, losses, opt_state = train_NDE_multipleics(ps, params, sts, NNs, truths, x₀s, train_data_plot, training_timeframe, S_scaling, density_contributions; sim_index=sim_indices, epoch=i, maxiter=maxiter, rule=optimizer)
