...
ps = ComponentArray(; wT=ps_wT, wS=ps_wS, baseclosure=ps_baseclosure)
...
function solve_NDE(ps, params, x₀, sts, NNs, timestep, Nt, timestep_multiple=10)
    ...
    for i in 2:Nt_solve
        ...
        predict_diffusivities!(νs, κs, Ris, ps.baseclosure)
...
function solve_NDE_postprocessing(ps, params, x₀, sts, NNs, timestep, Nt, timestep_multiple=2)
    ...
    for i in 2:Nt_solve
        ...
        Ris = calculate_Ri(zeros(coarse_size), zeros(coarse_size), ρ, Dᶠ, params.g, eos.reference_density, clamp_lims=(-Inf, Inf))
        _, κs = predict_diffusivities(Ris, ps.baseclosure)
...
function diagnose_fields(ps, params, x₀, sts, NNs, train_data_plot, timestep, Nt, timestep_multiple=2)
    ...
    ps_noNN.baseclosure .= ps.baseclosure
    ...
    νs, κs = predict_diffusivities(Ris, ps.baseclosure)
    νs_noNN, κs_noNN = predict_diffusivities(Ris_noNN, ps.baseclosure)
    ...
    for i in 1:size(wT_residuals, 2)
        ...
        wT_diffusive_boundarys[:, i], wS_diffusive_boundarys[:, i] = predict_diffusive_boundary_flux_dimensional(Ris[:, i], sols.T[:, i], sols.S[:, i], ps.baseclosure, params)        

        wT_diffusive_boundarys_noNN[:, i], wS_diffusive_boundarys_noNN[:, i] = predict_diffusive_boundary_flux_dimensional(Ris_truth[:, i], sols_noNN.T[:, i], sols_noNN.S[:, i], ps.baseclosure, params)
    end
...
# function train_NDE_stochastic(ps, params, sts, NNs, truths, x₀s, losses_prefactors, rng, train_data_plot; epoch=1, maxiter=2, rule=Optimisers.Adam())
...
#             sols = [diagnose_fields(ps_min, param, x₀, sts, NNs, data) for (data, x₀, param) in zip(train_data_plot.data, x₀s, params)]
...
function train_NDE_multipleics(ps, params, sts, NNs, truths, x₀s, train_data_plot, timeframes, S_scaling; sim_index=[1], epoch=1, maxiter=2, rule=Optimisers.Adam())
    ...
    for iter in 1:maxiter
        ...
        @printf("%s, Δt %s, round %d, iter %d/%d, loss average %6.10e, minimum loss %6.5e, ν_conv %6.5e, ν_shear %6.5e, m %6.5e, ΔRi %6.5e, Pr %6.5e, max NN weight %6.5e, gradient norm %6.5e\n",
                Dates.now(), prettytime(1e-9 * (time_ns() - wall_clock[1])), epoch, iter, maxiter, l, l_min,
                ps.baseclosure.ν_conv, ps.baseclosure.ν_shear, ps.baseclosure.m, ps.baseclosure.ΔRi, ps.baseclosure.Pr,
                maximum(abs, vcat(ps.wT, ps.wS)), maximum(abs, dps))
