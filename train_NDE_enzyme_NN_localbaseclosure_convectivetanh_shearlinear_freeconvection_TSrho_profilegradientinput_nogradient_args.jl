...
FILE_DIR = "./training_output/NDE_enzyme_$(args["hidden_layer"])layer_$(args["hidden_layer_size"])_$(args["activation"])_localbaseclosure_convectivetanh_shearlinear_TSrho_profilegradientinput_nogradient_args"
mkpath(FILE_DIR)
...
function individual_loss(ps, truth, params, x₀, ps_baseclosure, sts, NNs)
    ...
    return (; T=T_loss, S=S_loss, ρ=ρ_loss)
end

function loss(ps, truth, params, x₀, ps_baseclosure, sts, NNs, losses_prefactor=(; T=1, S=1, ρ=1))
...
function compute_loss_prefactor(individual_loss)
    T_loss, S_loss, ρ_loss = values(individual_loss)

    T_prefactor = 1
    S_prefactor = T_loss / S_loss
    ρ_prefactor = T_loss / ρ_loss

    return (T=T_prefactor, S=S_prefactor, ρ=ρ_prefactor)
end
...
function loss_multipleics(ps, truths, params, x₀s, ps_baseclosure, sts, NNs, losses_prefactor=(; T=1, S=1, ρ=1))
