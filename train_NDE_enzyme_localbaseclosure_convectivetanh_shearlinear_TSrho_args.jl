...
FILE_DIR = "./training_output/NDE_enzyme_localbaseclosure_convectivetanh_shearlinear_TSrho_args"
mkpath(FILE_DIR)
...
uw_NN = Chain(Dense(166, 1024, leakyrelu), Dense(1024, 31))
vw_NN = Chain(Dense(166, 1024, leakyrelu), Dense(1024, 31))
wT_NN = Chain(Dense(166, 1024, leakyrelu), Dense(1024, 31))
wS_NN = Chain(Dense(166, 1024, leakyrelu), Dense(1024, 31))
...
function loss_multipleics(ps, truths, params, x₀s, ps_baseclosure, sts, NNs, losses_prefactor=(; u=1, v=1, T=1, S=1, ρ=1, ∂u∂z=1, ∂v∂z=1, ∂T∂z=1, ∂S∂z=1, ∂ρ∂z=1))
    losses = [loss(ps, truth, param, x₀, ps_baseclosure, sts, NNs, losses_prefactor) for (truth, x₀, param) in zip(truths, x₀s, params)]
    return mean(losses)
end

loss_multipleics(ps, truths, params, x₀s, ps_baseclosure, sts, NNs, loss_prefactor)
...
# maxiters = [100]
# ps = ComponentArray(ν_conv=1.5, ν_shear=6.484e-02, m=-1.736e-01, Pr=1.1, ΔRi=0.1)
