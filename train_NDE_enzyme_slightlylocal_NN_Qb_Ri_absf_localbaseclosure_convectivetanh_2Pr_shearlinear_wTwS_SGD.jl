...
FILE_DIR = "./training_output/NDE_Qb_Ri_absf_wTwS_$(length(LES_FILE_DIRS))simnew_$(args["hidden_layer"])layer_$(args["hidden_layer_size"])_$(args["activation"])_mom_$(momentum_ratio)_localbaseclosure_2Pr_multichunk_SGD$(SGD_chunk_size)_test"
mkpath(FILE_DIR)
@info FILE_DIR
...
truths_plot = [(; u = data.profile.u.scaled, 
             v = data.profile.v.scaled, 
             T = data.profile.T.scaled, 
             S = data.profile.S.scaled, 
             ρ = data.profile.ρ.scaled, 
             ...
             ∂T∂z = data.profile.∂T∂z.scaled, 
             ∂S∂z = data.profile.∂S∂z.scaled, 
             ∂ρ∂z = data.profile.∂ρ∂z.scaled) for data in train_data_plot.data]
...
function predict_residual_flux(Ri, ∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, sts, NNs)
...
function solve_NDE(ps, params, x₀, ps_baseclosure, sts, NNs, Nt, timestep_multiple=10)
    ...
    for i in 2:Nt_solve
        ...
        wT_residual, wS_residual = predict_residual_flux(Ris, ∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, ps, params, sts, NNs)
...
function compute_loss_prefactor_density_contribution(individual_loss, contribution, S_scaling=1.0, momentum_ratio=0.25; u=true, v=true)
    ...
    uv_loss = maximum([u_loss + v_loss, 1e-5])
    ...
    ∂uv∂z_loss = maximum([∂u∂z_loss + ∂v∂z_loss, 1e-5])
...
function train_NDE_multipleics(ps, params, ps_baseclosure, sts, NNs, truths, x₀s, train_data_plot, timeframes, S_scaling, momentum_ratio, scaling_params; sim_index=[1], epoch=1, maxiter=2, rule=Optimisers.Adam())
...
function train_NDE_stochastic(ps, params, ps_baseclosure, sts, NNs, truths, x₀s, train_data_plot, timeframes, S_scaling, momentum_ratio, scaling_params, rng; indices_training=[1], epoch=1, maxiter=2, rule=Optimisers.Adam(), batchsize=1)
    opt_state = Optimisers.setup(rule, ps)
    opt_statemin = deepcopy(opt_state)
    l_min = Inf
    ps_min = deepcopy(ps)
...
for (i, (epoch, optimizer, maxiter, training_timeframe, plot_timeframe, training_index)) in enumerate(zip(end_epochs, optimizers, maxiters, training_timeframes, plot_timeframes, sim_indices))
    global ps = ps
    global sols = sols
    ps, losses, opt_state = train_NDE_stochastic(ps, params, ps_baseclosure, sts, NNs, truths, x₀s, train_data_plot, training_timeframe, S_scaling, momentum_ratio, scaling_params, rng; indices_training=training_index, epoch=i, maxiter=maxiter, rule=optimizer, batchsize=SGD_chunk_size)

    jldsave("$(FILE_DIR)/training_results_epoch$(epoch).jld2"; u=ps, losses=losses, state=opt_state, scaling=scaling_params, model=NNs, sts=sts)

    sols = [diagnose_fields(ps, param, x₀, ps_baseclosure, sts, NNs, data, length(plot_timeframe)) for (data, x₀, param) in zip(train_data_plot.data, x₀s_plot, params_plot)]
