...
#%%
NN_layers = vcat(Dense(11, hidden_layer_size, activation), [Dense(hidden_layer_size, hidden_layer_size, activation) for _ in 1:N_hidden_layer-1]..., Dense(hidden_layer_size, 1))
...
ps_NN .= glorot_uniform(rng, Float64, length(ps_NN))
...
function predict_residual_flux(Ri, ∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, sts, NNs)
    ...
    for i in 3:params.coarse_size-1
        x_wT = vcat(arctan_Ri[i-1:i+1], ∂T∂z_hat[i-1:i+1], ∂ρ∂z_hat[i-1:i+1], common_variables)
        x_wS = vcat(arctan_Ri[i-1:i+1], ∂S∂z_hat[i-1:i+1], ∂ρ∂z_hat[i-1:i+1], common_variables)
        
        wT[i] = first(NNs.NN(x_wT, p.NN, sts.NN))[1]
        wS[i] = first(NNs.NN(x_wS, p.NN, sts.NN))[1]
    end
