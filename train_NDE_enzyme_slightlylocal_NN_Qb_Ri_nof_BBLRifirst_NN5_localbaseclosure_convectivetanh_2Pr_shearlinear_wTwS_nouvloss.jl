...
#%%
NN_layers = vcat(Dense(21, hidden_layer_size, activation), [Dense(hidden_layer_size, hidden_layer_size, activation) for _ in 1:N_hidden_layer-1]..., Dense(hidden_layer_size, 1))
...
function predict_residual_flux(Ri, ∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, κs, T_top, S_top, p, params, sts, NNs)
    ...
    last_index = coarse_size - 2
    first_index = ifelse(nonbackground_κ_index == top_index, top_index, max(background_κ_index - grid_point_below_kappa + 1, 4))
    ...
    for i in first_index:last_index
        x = vcat(arctan_Ri[i-2:i+2], ∂T∂z_hat[i-2:i+2], ∂S∂z_hat[i-2:i+2], ∂ρ∂z_hat[i-2:i+2], common_variables)
...
optimizers = [Optimisers.Adam(learning_rate), Optimisers.Adam(learning_rate), Optimisers.Adam(learning_rate)]
