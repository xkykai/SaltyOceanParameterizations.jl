...
for (i, (epoch, optimizer, maxiter, training_timeframe)) in enumerate(zip(end_epochs, optimizers, maxiters, training_timeframes))
    ...
    for (i, data) in enumerate(validation_data_plot.data)
        ...
        animate_data(data, sol.sols_dimensional, sol.fluxes, sol.diffusivities, sol.sols_dimensional_noNN, sol.fluxes_noNN, sol.diffusivities_noNN, i, FILE_DIR, length(plot_timeframe); suffix="epoch$(epoch)_duration$(length(timeframes[1]))", prefix="validation")
    end
