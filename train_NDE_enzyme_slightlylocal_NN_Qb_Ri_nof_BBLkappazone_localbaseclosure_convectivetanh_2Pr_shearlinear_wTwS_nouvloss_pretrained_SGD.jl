...
PRETRAINED_NDE_DIR = "NDE_FC_Qb_Ri_nof_BBLkappazonelast55_trainFC19new_scalingtrain59new_2layer_256_relu_10seed_2Pr"
...
ps, scaling_params, sts, NNs = jldopen("./training_output/$(PRETRAINED_NDE_DIR)/training_results_epoch12000_end285.jld2", "r") do file
...
function compute_loss_prefactor_density_contribution(individual_loss, contribution, S_scaling=1.0)
    ...
    T_contribution = max(contribution.T, 1e-3)
    S_contribution = max(contribution.S, 1e-3)
...
function losses_multipleics(ps, truths, params, x₀s, ps_baseclosure, st, NN, losses_prefactor, Nt::Number, tstart=1, timestep_multiple=10)
    return [loss(ps, truth, param, x₀, ps_baseclosure, st, NN, Nt, tstart, timestep_multiple, loss_prefactor) for (truth, x₀, param, loss_prefactor) in zip(truths, x₀s, params, losses_prefactor)]
end

loss_multipleics(ps, truths, params, x₀s, ps_baseclosure, sts, NNs, loss_prefactors, length(25:10:285))
losses = losses_multipleics(ps, truths, params, x₀s, ps_baseclosure, sts, NNs, loss_prefactors, length(25:10:285))
...
optimizers = [Optimisers.Adam(learning_rate)]
maxiters = [5000]
training_timeframe = [timeframes[1][1:27]]
