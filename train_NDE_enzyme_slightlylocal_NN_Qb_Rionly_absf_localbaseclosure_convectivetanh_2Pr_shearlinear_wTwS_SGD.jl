...
#%%
NN_layers = vcat(Dense(8, hidden_layer_size, activation), [Dense(hidden_layer_size, hidden_layer_size, activation) for _ in 1:N_hidden_layer-1]..., Dense(hidden_layer_size, 1))
...
function predict_residual_flux(Ri, ∂T∂z_hat, ∂S∂z_hat, T_top, S_top, p, params, sts, NNs)
    ...
    for i in 3:params.coarse_size-1
        x_wT = vcat(arctan_Ri[i-1:i+1], ∂T∂z_hat[i-1:i+1], common_variables)
        x_wS = vcat(arctan_Ri[i-1:i+1], ∂S∂z_hat[i-1:i+1], common_variables)
        wT[i] = first(NNs.wT(x_wT, p.wT, sts.wT))[1]
        wS[i] = first(NNs.wS(x_wS, p.wS, sts.wS))[1]
    end
...
function solve_NDE(ps, params, x₀, ps_baseclosure, sts, NNs, Nt, timestep_multiple=10)
    ...
    for i in 2:Nt_solve
        ...
        wT_residual, wS_residual = predict_residual_flux(Ris, ∂T∂z_hat, ∂S∂z_hat, T_top, S_top, ps, params, sts, NNs)
