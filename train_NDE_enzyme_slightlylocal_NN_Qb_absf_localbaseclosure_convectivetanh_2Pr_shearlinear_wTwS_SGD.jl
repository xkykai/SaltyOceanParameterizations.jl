...
function predict_residual_flux(rss_shear_hat, ∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, sts, NNs)
    ...
    for i in 3:params.coarse_size-1
        x  = vcat(rss_shear_hat[i-1:i+1], ∂T∂z_hat[i-1:i+1], ∂S∂z_hat[i-1:i+1], ∂ρ∂z_hat[i-1:i+1], common_variables)
...
function solve_NDE(ps, params, x₀, ps_baseclosure, sts, NNs, Nt, timestep_multiple=10)
    ...
    rss_shear_hat = zeros(coarse_size+1)
    ...
    for i in 2:Nt_solve
        ...
        rss_shear_hat .= sqrt.(∂u∂z_hat .^ 2 + ∂v∂z_hat .^ 2)
        ...
        wT_residual, wS_residual = predict_residual_flux(rss_shear_hat, ∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, ps, params, sts, NNs)
