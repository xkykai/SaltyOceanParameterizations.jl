...
FILE_DIR = "./training_output/NDE_1NN_FC_Qb_absf_$(length(LES_FILE_DIRS))simnew_$(args["hidden_layer"])layer_$(args["hidden_layer_size"])_$(args["activation"])_2Pr_multichunk_SGD$(SGD_chunk_size)"
mkpath(FILE_DIR)
@info FILE_DIR
...
timeframe_chunk = [[25:10:65 for data in field_dataset], 
                   [55:10:95 for data in field_dataset], 
                   [85:10:125 for data in field_dataset], 
                   [115:10:155 for data in field_dataset],
                   [145:10:185 for data in field_dataset],
                   [175:10:215 for data in field_dataset],
...
#%%
NN_layers = vcat(Dense(11, hidden_layer_size, activation), [Dense(hidden_layer_size, hidden_layer_size, activation) for _ in 1:N_hidden_layer-1]..., Dense(hidden_layer_size, 2))
...
x₀s_plot = [(; T=data.profile.T.scaled[:, 1], S=data.profile.S.scaled[:, 1]) for data in train_data_plot.data]

scaling_params = write_scaling_params(scaling)

function predict_residual_flux(∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, st, NN)
    ...
    for i in 3:params.coarse_size-1
        wT[i], wS[i] = first(NN(vcat(∂T∂z_hat[i-1:i+1], ∂S∂z_hat[i-1:i+1], ∂ρ∂z_hat[i-1:i+1], common_variables), p, st))
    end
...
function solve_NDE(ps, params, x₀, ps_baseclosure, st, NN, Nt, timestep_multiple=10)
    eos = TEOS10EquationOfState()
    ...
    for i in 2:Nt_solve
        ...
        wT_residual, wS_residual = predict_residual_flux(∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, ps, params, st, NN)
...
sol_T, sol_S, sol_ρ = solve_NDE(ps, params[1], x₀s[1], ps_baseclosure, st, NN, length(25:10:45))
...
function individual_loss(ps, truth, params, x₀, ps_baseclosure, st, NN, Nt, tstart=1, timestep_multiple=10)
    Dᶠ = params.Dᶠ
    scaling = params.scaling
    sol_T, sol_S, sol_ρ = solve_NDE(ps, params, x₀, ps_baseclosure, st, NN, Nt, timestep_multiple)
...
# loss(ps, truths[1], params[1], x₀s[1], ps_baseclosure, st, NN, length(25:10:45))
...
ind_losses = [individual_loss(ps, truth, param, x₀, ps_baseclosure, st, NN, length(timeframe)) for (truth, x₀, param, timeframe) in zip(truths, x₀s, params, timeframes)]
...
# loss_multipleics(ps, [truths[1]], [params[1]], [x₀s[1]], ps_baseclosure, st, NN, [loss_prefactors[1]], length(25:10:45))
...
function predict_residual_flux_dimensional(∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, st, NN)
    wT_hat, wS_hat = predict_residual_flux(∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, st, NN)
...
function diagnose_fields(ps, params, x₀, ps_baseclosure, st, NN, train_data_plot, Nt, timestep_multiple=2)
    sols = solve_NDE(ps, params, x₀, ps_baseclosure, st, NN, Nt, timestep_multiple)

    ps_noNN = deepcopy(ps) .= 0
    sols_noNN = solve_NDE(ps_noNN, params, x₀, ps_baseclosure, st, NN, Nt, timestep_multiple)
    ...
    for i in 1:size(wT_residuals, 2)
        wT_residuals[:, i], wS_residuals[:, i] = predict_residual_flux_dimensional(∂T∂z_hats[:, i], ∂S∂z_hats[:, i], ∂ρ∂z_hats[:, i], T_tops[i], S_tops[i], ps, params, st, NN)
...
function train_NDE_multipleics(ps, params, ps_baseclosure, st, NN, truths, x₀s, train_data_plot, timeframes, S_scaling, scaling_params; sim_index=[1], epoch=1, maxiter=2, rule=Optimisers.Adam())
    ...
    ind_losses = [individual_loss(ps, truth, param, x₀, ps_baseclosure, st, NN, length(timeframes)) for (truth, x₀, param) in zip(truths[sim_index], x₀s[sim_index], params[sim_index])]
    loss_prefactors = compute_loss_prefactor_density_contribution.(ind_losses, compute_density_contribution.(train_data.data), S_scaling)
    ...
    for iter in 1:maxiter
        ...
        if iter % 1000 == 0
            ...
            # sols = [diagnose_fields(ps_min, param, x₀, ps_baseclosure, st, NN, data) for (data, x₀, param) in zip(train_data_plot.data, x₀s, params)]
...
function train_NDE_stochastic(ps, params, ps_baseclosure, st, NN, truths, x₀s, train_data_plot, timeframes, S_scaling, scaling_params, rng; indices_training=[1], epoch=1, maxiter=2, rule=Optimisers.Adam(), batchsize=1)
    opt_state = Optimisers.setup(rule, ps)
    opt_statemin = deepcopy(opt_state)
    l_min = Inf
    ps_min = deepcopy(ps)
    ...
    losses = zeros(maxiter)
    mean_loss = mean(losses)
    ind_losses = [individual_loss(ps, truth, param, x₀, ps_baseclosure, st, NN, length(timeframe)) for (truth, x₀, param, timeframe) in zip(truths[indices_training], x₀s[indices_training], params[indices_training], timeframes)]
    loss_prefactors = compute_loss_prefactor_density_contribution.(ind_losses, compute_density_contribution.(train_data.data), S_scaling)
...
for (i, (epoch, optimizer, maxiter, training_timeframe, plot_timeframe, training_index)) in enumerate(zip(end_epochs, optimizers, maxiters, training_timeframes, plot_timeframes, sim_indices))
    ...
    for (i, index) in enumerate(plot_indices)
        sol = sols[i]
        animate_data(train_data_plot.data[index], sol.sols_dimensional, sol.fluxes, sol.diffusivities, sol.sols_dimensional_noNN, sol.fluxes_noNN, sol.diffusivities_noNN, index, FILE_DIR, length(plot_timeframe); suffix="epoch$(epoch)")
    end

    plot_loss(losses, FILE_DIR; suffix="epoch$(epoch)")
