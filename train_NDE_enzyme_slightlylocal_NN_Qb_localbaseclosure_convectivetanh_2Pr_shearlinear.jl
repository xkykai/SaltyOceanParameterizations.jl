...
FILE_DIR = "./training_output/NDE_Qb_$(length(LES_FILE_DIRS))simnew_$(args["hidden_layer"])layer_$(args["hidden_layer_size"])_$(args["activation"])_mom_$(momentum_ratio)_localbaseclosure_2Pr_test"
mkpath(FILE_DIR)
@info FILE_DIR

BASECLOSURE_FILE_DIR = "./training_output/20sim_mom_1.0_localbaseclosure_convectivetanh_shearlinear_2Pr_EKI/training_results_mean.jld2"
...
#%%
NN_layers = vcat(Dense(19, hidden_layer_size, activation), [Dense(hidden_layer_size, hidden_layer_size, activation) for _ in 1:N_hidden_layer-1]..., Dense(hidden_layer_size, 1))
...
function predict_residual_flux(∂u∂z_hat, ∂v∂z_hat, ∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, sts, NNs)
    ...
    common_variables = vcat(params.uw.scaled.top, params.vw.scaled.top, wb_top_scaled, params.f_scaled)
...
function solve_NDE(ps, params, x₀, ps_baseclosure, sts, NNs, Nt, timestep_multiple=10)
    ...
    for i in 2:Nt_solve
        ...
        uw_residual, vw_residual, wT_residual, wS_residual = predict_residual_flux(∂u∂z_hat, ∂v∂z_hat, ∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, ps, params, sts, NNs)
...
function predict_residual_flux_dimensional(∂u∂z_hat, ∂v∂z_hat, ∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, sts, NNs)
    uw_hat, vw_hat, wT_hat, wS_hat = predict_residual_flux(∂u∂z_hat, ∂v∂z_hat, ∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, sts, NNs)
...
function diagnose_fields(ps, params, x₀, ps_baseclosure, sts, NNs, train_data_plot, Nt, timestep_multiple=2)
    ...
    for i in 1:size(wT_residuals, 2)
        uw_residuals[:, i], vw_residuals[:, i], wT_residuals[:, i], wS_residuals[:, i] = predict_residual_flux_dimensional(∂u∂z_hats[:, i], ∂v∂z_hats[:, i], ∂T∂z_hats[:, i], ∂S∂z_hats[:, i], ∂ρ∂z_hats[:, i], T_tops[i], S_tops[i], ps, params, sts, NNs)
...
for (i, (epoch, optimizer, maxiter, training_timeframe, plot_timeframe)) in enumerate(zip(end_epochs, optimizers, maxiters, training_timeframes, plot_timeframes))
    global ps = ps
    global sols = sols
    ps, losses, opt_state = train_NDE_multipleics(ps, params, ps_baseclosure, sts, NNs, truths, x₀s, train_data_plot, training_timeframe, S_scaling, momentum_ratio; sim_index=sim_indices, epoch=i+offset, maxiter=maxiter, rule=optimizer)
    ...
    jldsave("$(FILE_DIR)/training_results_epoch$(epoch)_end$(training_timeframe[end]).jld2"; u=ps, losses=losses, state=opt_state, model=NNs, sts=sts, scaling=scaling)
