...
dirname = "NDE$(NN_grid_points)_FC_Qb_nof_BBLRifirst$(grid_point_below_kappa)_$(LES_suite_name)_scaling$(scaling_LES_suite_name)_$(validation_LES_suite_name)_$(args["hidden_layer"])layer_$(args["hidden_layer_size"])_$(args["activation"])_$(seed)seed_2Pr_multichunk"
...
function train_NDE_multipleics(ps, params, ps_baseclosure, sts, NNs, truths, x₀s, train_data, truths_validation, x₀s_validation, params_validation, validation_data, train_data_plot, timeframes, S_scaling, scaling_params; sim_index=[1], epoch=1, maxiter=2, rule=Optimisers.Adam())
    ...
    WEIGHTS_FILE_PATH = "$(WEIGHTS_DIR)/model_weights_round$(epoch)_duration$(length(timeframes[1])).jld2"
    LOSS_TRAINING_FILE_PATH = "$(FILE_DIR)/model_loss_training_round$(epoch)_duration$(length(timeframes[1])).jld2"
    LOSS_VALIDATION_FILE_PATH = "$(FILE_DIR)/model_loss_validation_round$(epoch)_duration$(length(timeframes[1])).jld2"

    jldopen(WEIGHTS_FILE_PATH, "w") do file
        file["0"] = ps
    ...
    jldopen(LOSS_TRAINING_FILE_PATH, "w")
    jldopen(LOSS_VALIDATION_FILE_PATH, "w")
    ...
    for iter in 1:maxiter
        ...
        jldopen(WEIGHTS_FILE_PATH, "a") do file
            file["$(iter)"] = ps
        end

        jldopen(LOSS_TRAINING_FILE_PATH, "a+") do file
            file["$(iter)"] = l
        end

        jldopen(LOSS_VALIDATION_FILE_PATH, "a+") do file
            file["$(iter)"] = l_validation
        end

        if iter % 1000 == 0
            @info "Saving intermediate results"
            jldsave("$(FILE_DIR)/intermediate_training_results_round$(epoch)_epoch$(iter)_duration$(length(timeframes[1])).jld2"; 
...
for (i, (epoch, optimizer, maxiter, training_timeframe)) in enumerate(zip(end_epochs, optimizers, maxiters, training_timeframes))
    ...
    plot_loss(losses, FILE_DIR; suffix="epoch$(epoch)_duration$(length(timeframes[1]))")
end