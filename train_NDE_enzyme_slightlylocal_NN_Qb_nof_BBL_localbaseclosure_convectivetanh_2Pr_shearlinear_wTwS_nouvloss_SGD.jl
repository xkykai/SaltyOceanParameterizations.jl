...
FILE_DIR = "./training_output/NDE_Qb_dt20min_nof_BBL_wTwS_$(length(LES_FILE_DIRS))simnew_$(args["hidden_layer"])layer_$(args["hidden_layer_size"])_$(args["activation"])_$(seed)seed_$(learning_rate)lr_localbaseclosure_2Pr_6simstableRi_SGD"
mkpath(FILE_DIR)
@info FILE_DIR
...
for (i, (epoch, optimizer, maxiter, training_timeframe, plot_timeframe, training_index)) in enumerate(zip(end_epochs, optimizers, maxiters, training_timeframes, plot_timeframes, sim_indices))
    global ps = ps
    global sols = sols
    ps, losses, opt_state = train_NDE_stochastic(ps, params, ps_baseclosure, sts, NNs, truths, x₀s, train_data_plot, training_timeframe, S_scaling, scaling_params, rng; indices_training=training_index, epoch=i, maxiter=maxiter, rule=optimizer, batchsize=SGD_chunk_size)
