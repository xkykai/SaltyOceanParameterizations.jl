...
function predict_residual_flux(∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, sts, NNs)
    ...
    common_variable = wb_top_scaled

    wT = zeros(params.coarse_size+1)
    wS = zeros(params.coarse_size+1)

    for i in 3:params.coarse_size-1
        x  = vcat(∂T∂z_hat[i-1:i+1], ∂S∂z_hat[i-1:i+1], ∂ρ∂z_hat[i-1:i+1], common_variable)
...
function predict_residual_flux_dimensional(∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, sts, NNs)
    wT_hat, wS_hat = predict_residual_flux(∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, T_top, S_top, p, params, sts, NNs)
...
function lr_n_cycle_linear(current_iter::Int, n_cycles::Int, total_iters::Int, max_lr::Float64, min_lr::Float64)
    # If it's the first iteration, return min_lr
    if current_iter == 1
        return min_lr
    end

    # Calculate the length of one cycle
    cycle_length = total_iters ÷ n_cycles
    
    # Determine which cycle we're currently in
    current_cycle = (current_iter - 1) ÷ cycle_length + 1
    
    # Calculate the position within the current cycle
    position_in_cycle = (current_iter - 1) % cycle_length
    
    # Calculate the progress within the current cycle (0 to 1)
    progress = position_in_cycle / cycle_length
    
    # Determine if we're in the increasing or decreasing phase of the cycle
    if progress <= 0.5
        # Increasing phase
        rate = min_lr + (max_lr - min_lr) * (2 * progress)
    else
        # Decreasing phase
        rate = max_lr - (max_lr - min_lr) * (2 * (progress - 0.5))
    end
...
function train_NDE_multipleics(ps, params, ps_baseclosure, sts, NNs, truths, x₀s, train_data_plot, timeframes, S_scaling, scaling_params; sim_index=[1], epoch=1, maxiter=2, rule=Optimisers.Adam())
    ...
    for iter in 1:maxiter
        ...
        @printf("%s, Δt %s, round %d, iter %d/%d, loss average %6.10e, minimum loss %6.5e, max NN weight %6.5e, gradient norm %6.5e, learning rate %6.5e\n",
                Dates.now(), prettytime(1e-9 * (time_ns() - wall_clock[1])), epoch, iter, maxiter, l, l_min,
                maximum(abs, ps), maximum(abs, dps), lr)
...
optimizers = [Optimisers.Adam(learning_rate), Optimisers.Adam(learning_rate), Optimisers.Adam(learning_rate), Optimisers.Adam(learning_rate), Optimisers.Adam(learning_rate), Optimisers.Adam(learning_rate), Optimisers.Adam(learning_rate)]
...
# maxiters = [5]
# training_timeframes = [[timeframes[1][1:5] for _ in 1:length(field_datasets)]]

end_epochs = cumsum(maxiters)
sim_indices = collect(1:length(field_datasets))
...
for (i, (epoch, optimizer, maxiter, training_timeframe, plot_timeframe)) in enumerate(zip(end_epochs, optimizers, maxiters, training_timeframes, plot_timeframes))
    global ps = ps
    global sols = sols
    ps, losses, opt_state = train_NDE_stochastic(ps, params, ps_baseclosure, sts, NNs, truths, x₀s, train_data_plot, training_timeframe, S_scaling, scaling_params, rng; indices_training=sim_indices, epoch=i, maxiter=maxiter, rule=optimizer, batchsize=SGD_chunk_size)
