...
#%%
NN_layers = vcat(Dense(24, hidden_layer_size, activation), [Dense(hidden_layer_size, hidden_layer_size, activation) for _ in 1:N_hidden_layer-1]..., Dense(hidden_layer_size, 1))
...
function predict_residual_flux(∂T∂z_hat, ∂S∂z_hat, ∂ρ∂z_hat, p, params, sts, NNs)
    ...
    for i in 5:params.coarse_size-3
        wT[i] = first(NNs.wT(vcat(∂T∂z_hat[i-3:i+3], ∂S∂z_hat[i-3:i+3], ∂ρ∂z_hat[i-3:i+3], common_variables), p.wT, sts.wT))[1]
        wS[i] = first(NNs.wS(vcat(∂T∂z_hat[i-3:i+3], ∂S∂z_hat[i-3:i+3], ∂ρ∂z_hat[i-3:i+3], common_variables), p.wS, sts.wS))[1]
    end
