...
BASECLOSURE_FILE_DIR = "./training_output/nonlocalbaseclosure_1.0Sscaling_convectivetanh_shearlinear_const_TSrho_EKI_smallrho/training_results_mean.jld2"
...
#%%
NN_layers = vcat(Dense(14, hidden_layer_size, activation), [Dense(hidden_layer_size, hidden_layer_size, activation) for _ in 1:N_hidden_layer-1]..., Dense(hidden_layer_size, 1))
...
function predict_residual_flux(T_hat, S_hat, ∂ρ∂z_hat, p, params, sts, NNs)
    ...
    for i in 3:params.coarse_size-1
        wT[i] = first(NNs.wT(vcat(T_hat[i-2:i+1], S_hat[i-2:i+1], ∂ρ∂z_hat[i-1:i+1], common_variables), p.wT, sts.wT))[1]
        wS[i] = first(NNs.wS(vcat(T_hat[i-2:i+1], S_hat[i-2:i+1], ∂ρ∂z_hat[i-1:i+1], common_variables), p.wS, sts.wS))[1]
    end

    return wT, wS
end

function predict_residual_flux!(wT_residual, wS_residual, x′s, p, sts, NNs)
    for (i, x) in enumerate(x′s)
        wT_residual[i+2] = NNs.wT(x, p.wT, sts.wT)[1][1]
        wS_residual[i+2] = NNs.wT(x, p.wS, sts.wS)[1][1]
    end
    return nothing
end
...
function solve_NDE(ps, params, x₀, ps_baseclosure, sts, NNs, timestep, Nt, timestep_multiple=10)
    ...
    x′s = [zeros(4 + 4 + 3 + 3) for _ in 1:coarse_size-3]

    for x in x′s
        x[12] = params.wT.scaled.top
        x[13] = params.wS.scaled.top
        x[14] = params.f_scaled
    end
    ...
    for i in 2:Nt_solve
        ...
        for (j, x) in enumerate(x′s)
            x[1:4] .= @view T_hat[j:j+3]
            x[5:8] .= @view S_hat[j:j+3]
            x[9:11] .= @view ∂ρ∂z_hat[j+1:j+3]
        end
        ...
        predict_diffusivities!(νs, κs, Ris, Ris_above, params.wρ.unscaled.top, ps_baseclosure)

        LHS .= Dᶜ_hat * (-κs .* Dᶠ_hat)
        LHS .*= -τ / H^2

        predict_residual_flux!(wT_residual, wS_residual, x′s, ps, sts, NNs)
...
function predict_diffusive_flux(Ris, Ris_above, Qρ, T_hat, S_hat, ps_baseclosure, params)
    _, κs = predict_diffusivities(Ris, Ris_above, Qρ, ps_baseclosure)
...
function predict_diffusive_boundary_flux_dimensional(Ris, Ris_above, Qρ, T_hat, S_hat, ps_baseclosure, params)
    _wT_diffusive, _wS_diffusive = predict_diffusive_flux(Ris, Ris_above, Qρ, T_hat, S_hat, ps_baseclosure, params)
...
function solve_NDE_postprocessing(ps, params, x₀, ps_baseclosure, sts, NNs, timestep, Nt, timestep_multiple=2)
    ...
    for i in 2:Nt_solve
        ...
        _, κs = predict_diffusivities(Ris, Ris_above, params.wρ.unscaled.top, ps_baseclosure)
...
function diagnose_fields(ps, params, x₀, ps_baseclosure, sts, NNs, train_data_plot, timestep, Nt, timestep_multiple=2)
    ...
    νs, κs = predict_diffusivities(Ris, Ris_above, params.wρ.unscaled.top, ps_baseclosure)
    νs_noNN, κs_noNN = predict_diffusivities(Ris_noNN, Ris_above_noNN, params.wρ.unscaled.top, ps_baseclosure)
    ...
    for i in 1:size(wT_residuals, 2)
        ...
        wT_diffusive_boundarys[:, i], wS_diffusive_boundarys[:, i] = predict_diffusive_boundary_flux_dimensional(Ris[:, i], Ris_above[:, i], params.wρ.unscaled.top, sols.T[:, i], sols.S[:, i], ps_baseclosure, params)        

        wT_diffusive_boundarys_noNN[:, i], wS_diffusive_boundarys_noNN[:, i] = predict_diffusive_boundary_flux_dimensional(Ris_noNN[:, i], Ris_above_noNN[:, i], params.wρ.unscaled.top, sols_noNN.T[:, i], sols_noNN.S[:, i], ps_baseclosure, params)
    end
