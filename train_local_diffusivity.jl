...
ps = ComponentArray(ν₁ = 0.1, Riᶜ = 0.25, ΔRi = 0.1, Pr = 1.0)

function train_NDE(train_data, ps; coarse_size=32, dev=cpu_device(), maxiter=10)
    ...
    callback = function (opt_state, l, pred, ind_loss, p)
        @printf("%s, Δt %s, iter %d/%d, loss total %6.10e, u %6.5e, v %6.5e, T %6.5e, S %6.5e, ρ %6.5e, ν₁ %6.3e, Riᶜ %6.3e, ΔRi %6.3e, Pr %6.3e \n",
                Dates.now(), prettytime(1e-9 * (time_ns() - wall_clock[1])), iter, maxiter, l, ind_loss.u, ind_loss.v, ind_loss.T, ind_loss.S, ind_loss.ρ,
                p.ν₁, p.Riᶜ, p.ΔRi, p.Pr)
        losses[iter+1] = l
...
res, loss, sols, fluxes, losses = train_NDE(train_data, ps, maxiter=500)

jldsave("$(FILE_DIR)/training_results_1.jld2"; res, loss, sols, fluxes, losses)

res, loss, sols, fluxes, losses = train_NDE(train_data, res.u, maxiter=500)
