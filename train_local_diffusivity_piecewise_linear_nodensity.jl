...
FILE_DIR = "./training_output/local_diffusivity_piecewise_linear_nodensity_noclamp_lossequal_reltol1e-5"
mkpath(FILE_DIR)
...
function optimize_parameters(train_data, train_data_plot, ps; coarse_size=32, dev=cpu_device(), maxiter=10, optimizer=OptimizationOptimisers.ADAM(0.01), solver=DP5(), Ri_clamp_lims=(-Inf, Inf))
    ...
    function loss_DE(p)
        ...
        loss = u_loss + v_loss + T_loss + S_loss

        individual_loss = (u=u_loss, v=v_loss, T=T_loss, S=S_loss)
    ...
    callback = function (opt_state, l, pred, ind_loss, p)
        @printf("%s, Δt %s, iter %d/%d, loss total %6.10e, u %6.5e, v %6.5e, T %6.5e, S %6.5e, ν₁ %6.3e, m %6.3e, Pr %6.3e \n",
                Dates.now(), prettytime(1e-9 * (time_ns() - wall_clock[1])), iter, maxiter, l, ind_loss.u, ind_loss.v, ind_loss.T, ind_loss.S,
                p.ν₁, p.m, p.Pr)
        losses[iter+1] = l
    ...
    for (i, sol) in enumerate(sols_posttraining)
        for t in eachindex(sol.t)
            uw, vw, wT, wS = predict_total_flux_dimensional(sol[:, t], res.u, params[i])
...
res, loss, sols, fluxes, losses, diffusivities = optimize_parameters(train_data, train_data_plot, res.u, maxiter=500, optimizer=OptimizationOptimisers.ADAM(0.002), Ri_clamp_lims=(-20, 20), solver=VCABM3())
