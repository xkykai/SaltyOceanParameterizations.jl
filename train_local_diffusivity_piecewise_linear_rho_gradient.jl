...
FILE_DIR = "./training_output/local_diffusivity_piecewise_linear_rho_noclamp_lossequal_gradient_SW_FC_largeinitialdiffusivity"
mkpath(FILE_DIR)
...
function optimize_parameters(train_data, train_data_plot, ps; coarse_size=32, dev=cpu_device(), maxiter=10, optimizer=OptimizationOptimisers.ADAM(0.01), solver=DP5(), Ri_clamp_lims=(-Inf, Inf))
    ...
    function compute_loss_prefactor(p)
        ...
        ∂u∂z_loss = mean(mean.([(data.profile.∂u∂z.scaled .- ∂u∂z_hat).^2 for (data, ∂u∂z_hat) in zip(train_data.data, ∂u∂z_hats)]))
        ∂v∂z_loss = mean(mean.([(data.profile.∂v∂z.scaled .- ∂v∂z_hat).^2 for (data, ∂v∂z_hat) in zip(train_data.data, ∂v∂z_hats)]))
        ∂ρ∂z_loss = mean(mean.([(data.profile.∂ρ∂z.scaled .- ∂ρ∂z_hat).^2 for (data, ∂ρ∂z_hat) in zip(train_data.data, ∂ρ∂z_hats)]))
