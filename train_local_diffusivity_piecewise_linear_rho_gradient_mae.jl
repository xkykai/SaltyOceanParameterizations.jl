...
FILE_DIR = "./training_output/local_diffusivity_piecewise_linear_rho_noclamp_lossequal_gradient_mae_BFGS_SW_FC_largeinitialdiffusivity"
mkpath(FILE_DIR)
...
function optimize_parameters(train_data, train_data_plot, ps; coarse_size=32, dev=cpu_device(), maxiter=10, optimizer=OptimizationOptimisers.ADAM(0.01), solver=DP5(), Ri_clamp_lims=(-Inf, Inf))
    ...
    function compute_loss_prefactor(p)
        ...
        u_loss = mean(mean.([abs.(data.profile.u.scaled .- u) for (data, u) in zip(train_data.data, us)]))
        v_loss = mean(mean.([abs.(data.profile.v.scaled .- v) for (data, v) in zip(train_data.data, vs)]))
        ρ_loss = mean(mean.([abs.(data.profile.ρ.scaled .- ρ) for (data, ρ) in zip(train_data.data, ρs)]))

        ∂u∂z_loss = mean(mean.([abs.(data.profile.∂u∂z.scaled .- ∂u∂z_hat) for (data, ∂u∂z_hat) in zip(train_data.data, ∂u∂z_hats)]))
        ∂v∂z_loss = mean(mean.([abs.(data.profile.∂v∂z.scaled .- ∂v∂z_hat) for (data, ∂v∂z_hat) in zip(train_data.data, ∂v∂z_hats)]))
        ...
        return (u=u_prefactor, v=v_prefactor, ρ=ρ_prefactor, ∂u∂z=∂u∂z_prefactor, ∂v∂z=∂v∂z_prefactor, ∂ρ∂z=∂ρ∂z_prefactor)
    end
    ...
    function loss_DE(p)
        ...
        ∂u∂z_loss = losses_prefactor.∂u∂z * mean(mean.([(data.profile.∂u∂z.scaled .- ∂u∂z_hat).^2 for (data, ∂u∂z_hat) in zip(train_data.data, ∂u∂z_hats)]))
        ∂v∂z_loss = losses_prefactor.∂v∂z * mean(mean.([(data.profile.∂v∂z.scaled .- ∂v∂z_hat).^2 for (data, ∂v∂z_hat) in zip(train_data.data, ∂v∂z_hats)]))
        ∂ρ∂z_loss = losses_prefactor.∂ρ∂z * mean(mean.([(data.profile.∂ρ∂z.scaled .- ∂ρ∂z_hat).^2 for (data, ∂ρ∂z_hat) in zip(train_data.data, ∂ρ∂z_hats)]))

        loss = u_loss + v_loss + ρ_loss + ∂u∂z_loss + ∂v∂z_loss + ∂ρ∂z_loss

        individual_loss = (u=u_loss, v=v_loss, ρ=ρ_loss, ∂u∂z=∂u∂z_loss, ∂v∂z=∂v∂z_loss, ∂ρ∂z=∂ρ∂z_loss)
    ...
    ∂u∂z_losses = fill(1e-8, maxiter+1)
    ∂v∂z_losses = fill(1e-8, maxiter+1)
    ∂ρ∂z_losses = fill(1e-8, maxiter+1)

    wall_clock = [time_ns()]

    callback = function (opt_state, l, pred, ind_loss, p)
        @printf("%s, Δt %s, iter %d/%d, loss total %6.10e, u %6.5e, v %6.5e, ρ %6.5e, ∂u∂z %6.5e, ∂v∂z %6.5e, ∂ρ∂z %6.5e, ν₁ %6.3e, m %6.3e, Pr %6.3e \n",
                Dates.now(), prettytime(1e-9 * (time_ns() - wall_clock[1])), iter, maxiter, l, ind_loss.u, ind_loss.v, ind_loss.ρ, ind_loss.∂u∂z, ind_loss.∂v∂z, ind_loss.∂ρ∂z,
                p.ν₁, p.m, p.Pr)
        losses[iter+1] = l
...
function plot_loss(losses, FILE_DIR; epoch=1)
    ...
    lines!(axindividualloss, losses.∂u∂z, label="∂u∂z")
    lines!(axindividualloss, losses.∂v∂z, label="∂v∂z")
    lines!(axindividualloss, losses.∂ρ∂z, label="∂ρ∂z")
...
res, loss, sols, fluxes, losses, diffusivities = optimize_parameters(train_data, train_data_plot, ps, maxiter=5, optimizer=OptimizationOptimJL.BFGS(), Ri_clamp_lims=(-Inf, Inf), solver=VCABM3())
