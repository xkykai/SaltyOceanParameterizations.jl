...
FILE_DIR = "./training_output/local_diffusivity_piecewise_linear_rho_noclamp_lossequal_SW_FC_largeinitialdiffusivity_mae"
mkpath(FILE_DIR)
...
function optimize_parameters(train_data, train_data_plot, ps; coarse_size=32, dev=cpu_device(), maxiter=10, optimizer=OptimizationOptimisers.ADAM(0.01), solver=DP5(), Ri_clamp_lims=(-Inf, Inf))
    ...
    function loss_DE(p)
        ...
        u_loss = losses_prefactor.u * mean(mean.([abs.(data.profile.u.scaled .- u) for (data, u) in zip(train_data.data, us)]))
        v_loss = losses_prefactor.v * mean(mean.([abs.(data.profile.v.scaled .- v) for (data, v) in zip(train_data.data, vs)]))
        ρ_loss = losses_prefactor.ρ * mean(mean.([abs.(data.profile.ρ.scaled .- ρ) for (data, ρ) in zip(train_data.data, ρs)]))
