...
for i in 1:N_iterations
    ...
    Threads.@threads for j in 1:N_ensemble
        ps_ensemble = ComponentArray(ν_conv=ps_eki[1, j], ν_shear=ps_eki[2, j], Riᶜ=ps_eki[3, j], Pr_conv=ps_eki[4, j], Pr_shear=ps_eki[5, j], ΔRi=ps_eki[6, j])
        G_ens[j] = loss_multipleics(ps_ensemble, truths, params, x₀s, loss_prefactors)
    end
    EKP.update_ensemble!(ensemble_kalman_process, G_ens)
    @printf("%s, Δt %s, iter %d/%d, loss average %6.10e, ν_conv %6.5e, ν_shear %6.5e, Riᶜ %6.5e, Pr_conv %6.5e, Pr_shear %6.5e, ΔRi %6.5e\n",
...
ensemble_losses = [loss_multipleics(ComponentArray(ν_conv=p[1], ν_shear=p[2], Riᶜ=p[3], Pr_conv=p[4], Pr_shear=p[5], ΔRi=p[6]), 
                                    truths, params, x₀s, loss_prefactors) for p in eachcol(final_ensemble)]

ensemble_min = final_ensemble[:, argmin(ensemble_losses)]

ps_final_min = ComponentArray(; ν_conv=ensemble_min[1], ν_shear=ensemble_min[2], Riᶜ=ensemble_min[3], Pr_conv=ensemble_min[4], Pr_shear=ensemble_min[5], ΔRi=ensemble_min[6])

ensemble_mean_loss = loss_multipleics(ComponentArray(ν_conv=ensemble_mean[1], ν_shear=ensemble_mean[2], Riᶜ=ensemble_mean[3], Pr_conv=ensemble_mean[4], Pr_shear=ensemble_mean[5], ΔRi=ensemble_mean[6]), 
                                      truths, params, x₀s, loss_prefactors)

ps_final_mean = ComponentArray(; ν_conv=ensemble_mean[1], ν_shear=ensemble_mean[2], Riᶜ=ensemble_mean[3], Pr_conv=ensemble_mean[4], Pr_shear=ensemble_mean[5], ΔRi=ensemble_mean[6])
