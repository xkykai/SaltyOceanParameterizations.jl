...
FILE_DIR = "./training_output/nonlocal_diffusivity_rotation_NDE_gradient_tanh_relu_clamplim_-20_20_fast"
@info FILE_DIR
mkpath(FILE_DIR)
...
NN = Chain(Dense(38, 128, tanh_fast), Dense(128, 62, relu))
...
function train_NDE(train_data, train_data_plot, NN, ps_NN, st_NN; coarse_size=32, dev=cpu_device(), maxiter=10, optimizer=OptimizationOptimisers.ADAM(0.001), solver=DP5(), Ri_clamp_lims=(-10, 10))
    ...
    function predict_diffusivities(Ris, p, st, params)
        ν₀ = 1e-5
        κ₀ = 1e-5
        f_scaled = params.f_scaled
        x′ = vcat(Ris, params.uw.scaled.top, params.vw.scaled.top, params.wT.scaled.top, params.wS.scaled.top, f_scaled)

        interior_size = coarse_size - 1
        diffusivities = first(NN(x′, p, st))

        νs = vcat(0, diffusivities[1:interior_size] ./ 10, 0) .+ ν₀
        κs = vcat(0, diffusivities[interior_size+1:2*interior_size] ./ 10, 0) .+ κ₀
        return νs, κs
    end
    ...
    function predict_diffusive_flux(x, p, params, st)
        ...
        νs, κs = predict_diffusivities(Ris, p, st, params)
    ...
    function predict_NDE(p)
        ...
        # sols = [Array(solve(prob, solver, saveat=param.scaled_time, sensealg=sensealg, reltol=1e-2)) for (param, prob) in zip(params, probs)]
    ...
    for (i, sol) in enumerate(sols_posttraining)
        for t in eachindex(sol.t)
            ...
            Ris_truth = calculate_Ri(train_data_plot.data[i].profile.u.unscaled[:, t], 
                                     ...
                                     params[i].Dᶠ, params[i].g, eos.reference_density, clamp_lims=Ri_clamp_lims)
            νs, κs = predict_diffusivities(Ris, res.u, st_NN, params[i])
...
res, loss, sols, fluxes, losses, diffusivities = train_NDE(train_data, train_data_plot, NN, ps_NN, st_NN, Ri_clamp_lims=(-20, 20), 
                                                        #    maxiter=20, optimizer=OptimizationOptimisers.ADAM(0.001), solver=VCABM3());
