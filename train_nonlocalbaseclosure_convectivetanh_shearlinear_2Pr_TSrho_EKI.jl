...
LES_FILE_DIRS = ["./LES2/$(file)/instantaneous_timeseries.jld2" for file in LES_suite["train81new"]]
const S_scaling = args["S_scaling"]
FILE_DIR = "./training_output/$(length(LES_FILE_DIRS))simnew_mom_1.0_nonlocalbaseclosure_convectivetanh_shearlinear_2Pr_EKI"
mkpath(FILE_DIR)
...
ps = ComponentArray(ν_conv=0.5, ν_shear=0.1, Riᶜ=0.25, Pr_conv=1., Pr_shear=1., ΔRi=1e-3, C_en=0.1, x₀=2e-2, Δx=8e-3)
...
function predict_diffusivities(Ris, Ris_above, ∂ρ∂z, Qρ, ps_baseclosure)
    νs = nonlocal_Ri_ν_convectivetanh_shearlinear_2Pr.(Ris, Ris_above, ∂ρ∂z, Qρ, ps_baseclosure.ν_conv, ps_baseclosure.ν_shear, ps_baseclosure.Riᶜ, ps_baseclosure.ΔRi, ps_baseclosure.C_en, ps_baseclosure.x₀, ps_baseclosure.Δx)
    κs = nonlocal_Ri_κ_convectivetanh_shearlinear_2Pr.(Ris, Ris_above, ∂ρ∂z, Qρ, ps_baseclosure.ν_conv, ps_baseclosure.ν_shear, ps_baseclosure.Riᶜ, ps_baseclosure.ΔRi, ps_baseclosure.Pr_conv, ps_baseclosure.Pr_shear, ps_baseclosure.C_en, ps_baseclosure.x₀, ps_baseclosure.Δx)
    return νs, κs
end
...
ComponentArray(ν_conv=0.5, ν_shear=0.1, Riᶜ=0.25, Pr_conv=1., Pr_shear=1., ΔRi=1e-3, C_en=0.1, x₀=2e-2, Δx=8e-3)
...
for i in 1:N_iterations
    ...
    Threads.@threads for j in 1:N_ensemble
        ps_ensemble = ComponentArray(ν_conv=ps_eki[1, j], ν_shear=ps_eki[2, j], Riᶜ=ps_eki[3, j], Pr_conv=ps_eki[4, j], Pr_shear=ps_eki[5, j], ΔRi=ps_eki[6, j], C_en=ps_eki[7, j], x₀=ps_eki[8, j], Δx=ps_eki[9, j])
        G_ens[j] = loss_multipleics(ps_ensemble, truths, params, x₀s, loss_prefactors)
    end
    EKP.update_ensemble!(ensemble_kalman_process, G_ens)
    @printf("%s, Δt %s, iter %d/%d, loss average %6.10e, ν_conv %6.5e, ν_shear %6.5e, Riᶜ %6.5e, Pr_conv %6.5e, Pr_shear %6.5e, ΔRi %6.5e, C_en %6.5e, x₀ %6.5e, Δx %6.5e\n",
            Dates.now(), prettytime(1e-9 * (time_ns() - wall_clock[1])), i, N_iterations, mean(G_ens), 
            mean(ps_eki[1, :]), mean(ps_eki[2, :]), mean(ps_eki[3, :]), mean(ps_eki[4, :]), mean(ps_eki[5, :]), mean(ps_eki[6, :]), mean(ps_eki[7, :]), mean(ps_eki[8, :]), mean(ps_eki[9, :]))
...
ensemble_losses = [loss_multipleics(ComponentArray(ν_conv=p[1], ν_shear=p[2], Riᶜ=p[3], Pr_conv=p[4], Pr_shear=p[5], ΔRi=p[6], C_en=p[7], x₀=p[8], Δx=p[9]), 
                                    truths, params, x₀s, loss_prefactors) for p in eachcol(final_ensemble)]

ensemble_min = final_ensemble[:, argmin(ensemble_losses)]

ps_final_min = ComponentArray(; ν_conv=ensemble_min[1], ν_shear=ensemble_min[2], Riᶜ=ensemble_min[3], Pr_conv=ensemble_min[4], Pr_shear=ensemble_min[5], ΔRi=ensemble_min[6], C_en=ensemble_min[7], x₀=ensemble_min[8], Δx=ensemble_min[9])

ensemble_mean_loss = loss_multipleics(ComponentArray(ν_conv=ensemble_mean[1], ν_shear=ensemble_mean[2], Riᶜ=ensemble_mean[3], Pr_conv=ensemble_mean[4], Pr_shear=ensemble_mean[5], ΔRi=ensemble_mean[6], C_en=ensemble_mean[7], x₀=ensemble_mean[8], Δx=ensemble_mean[9]), 
                                      truths, params, x₀s, loss_prefactors)

ps_final_mean = ComponentArray(; ν_conv=ensemble_mean[1], ν_shear=ensemble_mean[2], Riᶜ=ensemble_mean[3], Pr_conv=ensemble_mean[4], Pr_shear=ensemble_mean[5], ΔRi=ensemble_mean[6], C_en=ensemble_mean[7], x₀=ensemble_mean[8], Δx=ensemble_mean[9])
