...
using SaltyOceanParameterizations: calculate_Ri, nonlocal_Ri_ν_convectivetanh_shearlinear_const, nonlocal_Ri_κ_convectivetanh_shearlinear_const
using Oceananigans
using JLD2
...
FILE_DIR = "./training_output/nonlocalbaseclosure_$(S_scaling)Sscaling_convectivetanh_shearlinear_const_TSrho_EKI_smallrho"
mkpath(FILE_DIR)
...
ps = ComponentArray(ν_conv=1., ν_shear=6.484e-02, m=-1.736e-01, Pr=1.1, ΔRi=0.1, C_en=0.005)
...
function predict_diffusivities(Ris, Ris_above, Qρ, ps_baseclosure)
    νs = nonlocal_Ri_ν_convectivetanh_shearlinear_const.(Ris, Ris_above, Qρ, ps_baseclosure.ν_conv, ps_baseclosure.ν_shear, ps_baseclosure.m, ps_baseclosure.ΔRi, ps_baseclosure.C_en)
    κs = nonlocal_Ri_κ_convectivetanh_shearlinear_const.(νs, ps_baseclosure.Pr)
    return νs, κs
end

function predict_diffusivities!(νs, κs, Ris, Ris_above, Qρ, ps_baseclosure)
    νs .= nonlocal_Ri_ν_convectivetanh_shearlinear_const.(Ris, Ris_above, Qρ, ps_baseclosure.ν_conv, ps_baseclosure.ν_shear, ps_baseclosure.m, ps_baseclosure.ΔRi, ps_baseclosure.C_en)
    κs .= nonlocal_Ri_κ_convectivetanh_shearlinear_const.(νs, ps_baseclosure.Pr)
    return nothing
end
...
function solve_NDE(ps, params, x₀, timestep_multiple=10)
    ...
    for i in 2:Nt_solve
        ...
        predict_diffusivities!(νs, κs, Ris, Ris_above, params.wρ.unscaled.top, ps)
...
function predict_diffusive_flux(Ris, Ris_above, Qρ, u_hat, v_hat, T_hat, S_hat, ps_baseclosure, params)
    νs, κs = predict_diffusivities(Ris, Ris_above, Qρ, ps_baseclosure)
...
function predict_diffusive_boundary_flux_dimensional(Ris, Ris_above, Qρ, u_hat, v_hat, T_hat, S_hat, ps_baseclosure, params)
    _uw_diffusive, _vw_diffusive, _wT_diffusive, _wS_diffusive = predict_diffusive_flux(Ris, Ris_above, Qρ, u_hat, v_hat, T_hat, S_hat, ps_baseclosure, params)
...
function diagnose_fields(ps, params, x₀, train_data_plot, timestep_multiple=2)
    ...
    νs, κs = predict_diffusivities(Ris, Ris_above, params.wρ.unscaled.top, ps)
    ...
    for i in 1:size(wT_diffusive_boundarys, 2)
        uw_diffusive_boundarys[:, i], vw_diffusive_boundarys[:, i], wT_diffusive_boundarys[:, i], wS_diffusive_boundarys[:, i] = predict_diffusive_boundary_flux_dimensional(Ris[:, i], Ris_above[:, i], params.wρ.unscaled.top, sols.u[:, i], sols.v[:, i], sols.T[:, i], sols.S[:, i], ps, params)
    end
...
prior_loss = loss_multipleics(ps, truths, params, x₀s, loss_prefactors)

prior_ν_conv = constrained_gaussian("ν_conv", ps.ν_conv, 0.1, -Inf, Inf)
prior_ν_shear = constrained_gaussian("ν_shear", ps.ν_shear, 1e-2, -Inf, Inf)
prior_m = constrained_gaussian("m", ps.m, 3e-2, -Inf, Inf)
prior_Pr = constrained_gaussian("Pr", ps.Pr, 0.2, -Inf, Inf)
prior_ΔRi = constrained_gaussian("ΔRi", ps.ΔRi, 5e-4, -Inf, Inf)
prior_C_en = constrained_gaussian("C_en", ps.C_en, 1e-3, -Inf, Inf)

priors = combine_distributions([prior_ν_conv, prior_ν_shear, prior_m, prior_Pr, prior_ΔRi, prior_C_en])
target = [0.]
...
for i in 1:N_iterations
    ...
    Threads.@threads for j in 1:N_ensemble
        ps_ensemble = ComponentArray(ν_conv=ps_eki[1, j], ν_shear=ps_eki[2, j], m=ps_eki[3, j], Pr=ps_eki[4, j], ΔRi=ps_eki[5, j], C_en=ps_eki[6, j])
        G_ens[j] = loss_multipleics(ps_ensemble, truths, params, x₀s, loss_prefactors)
    end
    EKP.update_ensemble!(ensemble_kalman_process, G_ens)
    @printf("%s, Δt %s, iter %d/%d, loss average %6.10e, ν_conv %6.5e, ν_shear %6.5e, m %6.5e, Pr %6.5e, ΔRi %6.5e, C_en %6.5e\n",
            Dates.now(), prettytime(1e-9 * (time_ns() - wall_clock[1])), i, N_iterations, mean(G_ens), 
            mean(ps_eki[1, :]), mean(ps_eki[2, :]), mean(ps_eki[3, :]), mean(ps_eki[4, :]), mean(ps_eki[5, :]), mean(ps_eki[6, :]))
...
ensemble_losses = [loss_multipleics(ComponentArray(ν_conv=p[1], ν_shear=p[2], m=p[3], Pr=p[4], ΔRi=p[5], C_en=p[6]), 
                                    truths, params, x₀s, loss_prefactors) for p in eachcol(final_ensemble)]

ensemble_min = final_ensemble[:, argmin(ensemble_losses)]

ps_final_min = ComponentArray(; ν_conv=ensemble_min[1], ν_shear=ensemble_min[2], m=ensemble_min[3], Pr=ensemble_min[4], ΔRi=ensemble_min[5], C_en=ensemble_min[6])

ensemble_mean_loss = loss_multipleics(ComponentArray(ν_conv=ensemble_mean[1], ν_shear=ensemble_mean[2], m=ensemble_mean[3], Pr=ensemble_mean[4], ΔRi=ensemble_mean[5], C_en=ensemble_mean[6]), 
                                      truths, params, x₀s, loss_prefactors)

ps_final_mean = ComponentArray(; ν_conv=ensemble_mean[1], ν_shear=ensemble_mean[2], m=ensemble_mean[3], Pr=ensemble_mean[4], ΔRi=ensemble_mean[5], C_en=ensemble_mean[6])
